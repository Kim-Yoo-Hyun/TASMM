...
| Does this prove real navigation `SR` / `SPL`? | E002/E005 provide `ExpectedSearchCost` and `AttemptSPL` proxy. E007-M07 packages an occupancy-grid path-cost proxy table. E008-M22 verifies a tiny local `HM3D ObjectNav` + `Habitat` route. E008-M37 executes the dynamic-stale overlay smoke: detector confidence `SR` 1.0 / `SPL` 0.407894, fixed current top-k `SR` 0.5 / `SPL` 0.373373, H001 `SR` 0.5 / `SPL` 0.141996, static stale memory `SR` 0.0, task-agnostic memory trust `SR` 0.5 / `SPL` 0.167627. E008-M41 executes repaired budget-matched rows: H001 repaired `SR`/`SPL` 0.5/0.373373, detector/fixed current 0.5/0.373373, task-agnostic 0.5/0.373373, static stale 0.0/0.0. E008-M42 interprets the result and blocks scale-up. E008-M43 fixes a source-diverse redesign contract. E008-M44 materializes source-diverse rows and verifies leakage/budget readiness. E008-M45 fixes Docker-ready execution contract/preflight, but has no new trajectory execution. | No. M45 is contract/preflight, not a navigation result. Next evidence must come from E008-M46 Docker `Habitat` execution before larger scale and navigation/search baselines. |
...
- E008-M20 shows the immediate coverage blocker is resolved; E008-M21 fixes the runner contract and Docker/data preflight; E008-M22 produces detector-policy trajectory `SR` / `SPL` smoke rows. E008-M23 shows success consistency is good, but proxy `SPL` order is not a reliable replacement for trajectory `SPL`; E008-M24 fixes the source contract, E008-M25 materializes H001 source/plan rows, E008-M26 materializes H001 visit-order/path rows without goal leakage, E008-M27 evaluates those rows against eval-only goals without policy leakage, E008-M28 decides repair-before-trajectory, E008-M29 fixes the leakage-safe backstop/source-repair contract, E008-M30 replays it, E008-M31 fixes the trajectory input contract/source-gap boundary, E008-M32 executes the H001 fallback trajectory smoke, E008-M33 aligns it against detector trajectories, E008-M34 selects a counterfactual stale overlay contract before scale-up, E008-M35 materializes the overlay rows without policy-input leakage, E008-M36 adapts the generalized M37 trajectory runner input contract, E008-M37 executes the dynamic-stale overlay trajectory smoke, E008-M38 records a repair-before-scale decision, E008-M39 fixes the budget-matched repair/source-gap contract, E008-M40 materializes budget-matched repaired rows, E008-M41 executes them, E008-M42 blocks scale-up, E008-M43 fixes the source-diverse redesign contract, E008-M44 materializes source-diverse rows, and E008-M45 fixes the Docker-ready trajectory contract.
- Current final real navigation `SR` / `SPL` remains unsupported because H001 repaired matches detector/fixed current top-k and task-agnostic memory trust on `SR`/`SPL`, while source-gap rows still fail.
...
- The correct immediate route is E008-M46: execute the source-diverse budget-5 rows in Docker `Habitat` and verify that trajectory metrics remain leakage-safe.
...
- The next technical step should be E008-M46 source-diverse redesign trajectory execution smoke.
