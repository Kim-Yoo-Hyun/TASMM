#!/usr/bin/env python3
"""Execute E008-M40 budget-matched repair rows as an E008-M41 trajectory smoke."""
...
VERSION = "e008_m41_budget_matched_repair_trajectory_execution_smoke_v0"
READY_STATUS = "e008_m41_budget_matched_repair_trajectory_execution_smoke_ready"
BLOCKED_STATUS = "e008_m41_budget_matched_repair_trajectory_execution_smoke_blocked"
NEXT_UNIT = "E008-M42 budget-matched repair result interpretation and scale decision"

DEFAULT_M40_CONTRACT = EXP_ROOT / "artifacts" / "E008-M40_budget_matched_repair_row_materialization_smoke_v0"
DEFAULT_ARTIFACT_DIR = EXP_ROOT / "artifacts" / "E008-M41_budget_matched_repair_trajectory_execution_smoke_v0"
DEFAULT_DATA_OUT_DIR = ROOT / "local_dataset" / "HM3D_navigation_bridge" / "E008-M41_budget_matched_repair_trajectory_execution_smoke_v0"
...
def build_claim_boundary_rows(ready: bool) -> list[dict[str, Any]]:
    return [
        {
            "version": VERSION,
            "claim_id": "budget_matched_repair_trajectory_smoke",
            "supported": ready,
            "claim_boundary": "M41 executes the M40 budget-matched repaired rows in Habitat as a smoke test.",
        },
        {
            "version": VERSION,
            "claim_id": "final_real_navigation_sr_spl",
            "supported": False,
            "claim_boundary": "M41 is still a 6-episode counterfactual smoke; final navigation claim requires scale and navigation/search baselines.",
        },
        {
            "version": VERSION,
            "claim_id": "source_gap_policy_claim",
            "supported": False,
            "claim_boundary": "Source-gap rows remain a separate source-expansion boundary, not a pure policy-failure claim.",
        },
        {
            "version": VERSION,
            "claim_id": "human_intent_main_claim",
            "supported": False,
            "claim_boundary": "Structured task context remains a memory-trust condition, not a natural-language human-intent claim.",
        },
    ]
...
def build_route_decision_rows(ready: bool) -> list[dict[str, Any]]:
    return [
        {
            "version": VERSION,
            "decision": "interpret_budget_matched_repair_trajectory_result" if ready else "repair_m41_runner",
            "selected_next_unit": NEXT_UNIT if ready else "repair E008-M41 budget-matched repair trajectory execution smoke",
...
def build_report(coverage: dict[str, Any], aggregate_rows: list[dict[str, Any]], pairwise_rows: list[dict[str, Any]]) -> str:
    ...
    return "\n".join(
        [
            "# E008-M41 Budget-Matched Repair Trajectory Execution Smoke",
            "",
            ...
            m37.markdown_table(
                policy_rows,
                ["group_id", "success_rows", "scan_task_policy_rows", "SR", "SPL", "PathLengthM_mean", "OldLocationDeadEndCostM_mean"],
            ),
            "",
            ...
            "- Source-gap rows remain a separate source-expansion boundary.",
...
def main() -> None:
    ...
    m37.DEFAULT_M36_CONTRACT = DEFAULT_M40_CONTRACT
