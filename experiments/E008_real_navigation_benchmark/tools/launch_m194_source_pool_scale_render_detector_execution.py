#!/usr/bin/env python3
"""Launch E008-M194 source-pool scale render or detector jobs."""
...
VERSION = "e008_m194_source_pool_scale_render_detector_launch_v0"
...
def command_status(command: str) -> dict[str, Any]:
    ...
    except Exception as exc:  # noqa: BLE001 - launch artifact should record local failures.
...
def command_row(job_id: str) -> dict[str, Any]:
    for row in read_jsonl(M193_ARTIFACT_DIR / "long_job_command_rows.jsonl"):
...
def render_ready() -> bool:
    ...
    return str(coverage.get("status")) in {
        "e008_m194_render_ready",
        "e008_m194_render_ready_with_depth_filtered_frames",
    }


def detector_ready() -> bool:
    coverage = read_json(ARTIFACT_DIR / "detector_verification_coverage.json")
    return str(coverage.get("status")) == "e008_m194_detector_candidate_source_ready"
...
def build_report(coverage: dict[str, Any], preflight_rows: list[dict[str, Any]]) -> str:
    return "\n".join(
        [
            "# E008-M194 Source-Pool Scale Render/Detector Launch",
            "",
            "## Facts",
            "",
            ...
            *[
            ...
            "## Claim Boundary",
            "",
            "- M194 launch only starts source-pool scale render/detector jobs.",
            "- Candidate validation, goal evaluation, trajectory execution, and protected-baseline claims require downstream gates.",
            "",
        ]
    )
...
def main() -> int:
    ...
    m193 = read_json(M193_ARTIFACT_DIR / "coverage.json")
    ...
    render_row = command_row("E008-M194-render")
    detector_row = command_row("E008-M194-detector")
    if stage == "render":
        job_row = render_row
        ...
        preflight_rows = [
            {
                "gate_id": "m193_ready",
                "gate_status": "pass" if m193.get("m194_gate_ready") else "fail",
                "details": m193.get("status"),
            },
            {
                "gate_id": "render_plan_rows_ready",
                "gate_status": "pass" if len(render_plan_rows) == 960 else "warning" if render_plan_rows else "fail",
            ...
            {
                "gate_id": "long_job_command_present",
                "gate_status": "pass" if bool(render_row.get("command")) else "fail",
                "details": render_row.get("job_id"),
            },
        ]
    elif stage == "detector":
        ...
        preflight_rows = [
            ...
            {
                "gate_id": "long_job_command_present",
                "gate_status": "pass" if bool(detector_row.get("command")) else "fail",
                "details": detector_row.get("job_id"),
            },
        ]
    else:
        job_row = {}
    ...
    status = (
        f"e008_m194_{stage}_launched"
        if launch_executed
        else "e008_m194_already_ready"
        if stage == "none"
        else f"e008_m194_{stage}_launch_blocked_or_failed"
    )
    coverage = {
        "version": VERSION,
        ...
        "selected_next_unit": "E008-M194 completion verification",
