#!/usr/bin/env python3
"""Evaluate E008-M112 ConceptGraphs visit-order rows against ObjectNav targets."""
...
ARTIFACT_DIR = EXP_ROOT / "artifacts" / "E008-M113_conceptgraphs_hm3d_candidate_goal_evaluation_smoke_v0"
DATA_OUT_DIR = (
    ROOT
    / "local_dataset"
    / "HM3D_navigation_bridge"
    / "E008-M113_conceptgraphs_hm3d_candidate_goal_evaluation_smoke_v0"
)
...
M112_ARTIFACT_DIR = EXP_ROOT / "artifacts" / "E008-M112_conceptgraphs_hm3d_candidate_visit_order_path_smoke_v0"
...
VERSION = "e008_m113_conceptgraphs_hm3d_candidate_goal_evaluation_smoke_v0"
...
def build_query_goal_metric_rows(
    scan_metric_rows: list[dict[str, Any]],
    query_source_rows: list[dict[str, Any]],
) -> list[dict[str, Any]]:
    boundary_by_scan = {str(row.get("scan_id")): row for row in query_source_rows}
    ...
    for metric in scan_metric_rows:
        ...
        row.update(
            {
                "version": VERSION,
                "metric_scope": "conceptgraphs_query_policy_goal_eval",
                "query_uid": boundary.get("query_uid"),
                ...
                "claim_boundary": "M113 scores fixed M112 ConceptGraphs visit-order rows against ObjectNav goals/viewpoints only after policy order is frozen.",
            }
        )
        out.append(row)
    return out
        ...
        enriched.update(
            {
                ...
                "source_ready_after_m111": bool(boundary.get("source_ready")),
                "source_class_name_boundary": "generic_item_class_names_clip_text_score_only",
                "claim_boundary": "M113 failure rows are leakage-safe source-gap recovery diagnostics, not executed navigation failures.",
            }
        )
        out.append(enriched)
    return out
...
def build_claim_boundary_rows() -> list[dict[str, Any]]:
    return [
        {
            "version": VERSION,
            "claim_id": "supported_conceptgraphs_leakage_safe_goal_eval_proxy",
            "supported": True,
            "claim_boundary": "M113 joins fixed M112 ConceptGraphs visit-order rows to ObjectNav targets only as evaluation labels.",
        },
        {
            "version": VERSION,
            "claim_id": "unsupported_real_navigation_sr_spl",
            "supported": False,
            "claim_boundary": "M113 is not a Habitat trajectory execution and cannot claim real navigation SR/SPL.",
        },
        {
            "version": VERSION,
            "claim_id": "unsupported_final_rgbd_open_vocab_robustness",
            "supported": False,
            "claim_boundary": "M113 covers two source-gap ConceptGraphs cases and does not provide heldout transfer or external navigation/search baselines.",
        },
        {
            "version": VERSION,
            "claim_id": "unsupported_conceptgraphs_class_name_recognition",
            "supported": False,
            "claim_boundary": "M113 uses CLIP feature/text ranked ConceptGraphs candidates; current source class names are generic item labels.",
        },
        {
            "version": VERSION,
            "claim_id": "unsupported_human_intent_main_claim",
            "supported": False,
            "claim_boundary": "M113 uses a source-gap recovery probe context and does not test structured human intent as a main effect.",
        },
    ]
...
def build_route_decision_rows(ready: bool, any_primary_success: bool) -> list[dict[str, Any]]:
    if ready:
        return [
            {
                "version": VERSION,
                "decision": "conceptgraphs_goal_eval_smoke_ready_but_navigation_claim_blocked",
                "selected_next_unit": "E008-M114 ConceptGraphs HM3D goal-evaluation result interpretation and trajectory decision",
                "reason": "M113 evaluates frozen ConceptGraphs visit-order rows against ObjectNav targets without policy leakage; next step is to interpret source-gap recovery and decide whether trajectory execution is justified.",
    ...
    return [
        {
            "version": VERSION,
            "decision": "repair_m113_conceptgraphs_goal_evaluation_smoke",
            "selected_next_unit": "repair E008-M113 ConceptGraphs goal-evaluation smoke",
            "reason": "M113 rows are incomplete or use blocked eval-only fields as policy inputs.",
...
def build_report(
    ...
    query_goal_metric_rows: list[dict[str, Any]],
    failure_rows: list[dict[str, Any]],
) -> str:
    aggregate_lines = []
    ...
    query_lines = []
    for row in query_goal_metric_rows:
        query_lines.append(
    ...
    return f"""# E008-M113 ConceptGraphs HM3D Candidate Goal-Evaluation Smoke
...
- Query-policy rows: {coverage['query_policy_metric_rows']}.
...
- M113 uses `ObjectNav` goal/viewpoint fields only as evaluation labels.
- M113 reports `GoalEvalProxySR` / `GoalEvalProxySPL` style diagnostics, not real navigation `SR` / `SPL`.
- M113 covers only two `ConceptGraphs` source-gap cases.
- M113 does not claim final real RGB-D/open-vocabulary robustness or class-name recognition.
- M113 does not make human intent a main claim.
"""
...
def main() -> None:
    ...
    m112_coverage = read_json(M112_ARTIFACT_DIR / "coverage.json")
    ...
    nav_rows = read_jsonl(M111_ARTIFACT_DIR / "candidate_navmesh_validation_rows.jsonl")
    visit_rows = read_jsonl(M112_ARTIFACT_DIR / "candidate_visit_order_rows.jsonl")
    if not episode_rows:
        raise SystemExit("missing M64 val_mini_episode_rows.jsonl")
    if not query_source_rows:
        raise SystemExit("missing M111 query_source_boundary_rows.jsonl")
    if not nav_rows:
        raise SystemExit("missing M111 candidate_navmesh_validation_rows.jsonl")
    if not visit_rows:
        raise SystemExit("missing M112 candidate_visit_order_rows.jsonl")

    target_episode_ids = {str(row.get("adapter_episode_id")) for row in query_source_rows}
    ...
    query_goal_metric_rows = build_query_goal_metric_rows(scan_metric_rows, query_source_rows)
    failure_rows = enrich_failure_rows(m12.build_failure_rows(scan_metric_rows), query_source_rows)
    ...
    ready = (
        bool(aggregate_rows)
        and len(goal_rows) == len(query_source_rows)
        and len(query_goal_metric_rows) == len(query_source_rows) * len(aggregate_rows)
        and leakage_pass
    ...
    coverage = {
        ...
        if ready
        else "e008_m113_conceptgraphs_hm3d_candidate_goal_evaluation_smoke_blocked",
        "generated_at": datetime.now().isoformat(timespec="seconds"),
        "m112_status": m112_coverage.get("status"),
        "artifact_output_root": str(ARTIFACT_DIR),
        "derived_output_root": str(DATA_OUT_DIR),
        "query_rows": len(query_source_rows),
        "eval_episode_rows": len(goal_rows),
        "expected_eval_episode_rows": len(query_source_rows),
        ...
        "query_policy_metric_rows": len(scan_metric_rows),
        "conceptgraphs_query_goal_metric_rows": len(query_goal_metric_rows),
    ...
    for output_dir in (ARTIFACT_DIR, DATA_OUT_DIR):
        ...
        write_jsonl(output_dir / "conceptgraphs_eval_goal_rows.jsonl", goal_rows)
        ...
        write_jsonl(output_dir / "conceptgraphs_query_goal_metric_rows.jsonl", query_goal_metric_rows)
    ...
    (ARTIFACT_DIR / "report.md").write_text(
        build_report(coverage, aggregate_rows, query_goal_metric_rows, failure_rows),
        encoding="utf-8",
    )
