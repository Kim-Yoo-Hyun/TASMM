#!/usr/bin/env python3
"""Evaluate E008-M88 source-gap detector visit-order rows against ObjectNav targets."""
...
ARTIFACT_DIR = EXP_ROOT / "artifacts" / "E008-M89_source_gap_detector_candidate_goal_evaluation_smoke_v0"
DATA_OUT_DIR = ROOT / "local_dataset" / "HM3D_navigation_bridge" / "E008-M89_source_gap_detector_candidate_goal_evaluation_smoke_v0"
...
M88_ARTIFACT_DIR = EXP_ROOT / "artifacts" / "E008-M88_source_gap_detector_candidate_visit_order_path_smoke_v0"
...
VERSION = "e008_m89_source_gap_detector_candidate_goal_evaluation_smoke_v0"
...
def build_source_gap_case_goal_metric_rows(
    ...
    for metric in scan_metric_rows:
        ...
        row.update(
            {
                "version": VERSION,
                "metric_scope": "source_gap_case_policy_goal_eval",
                ...
                "claim_boundary": "M89 scores source-gap detector candidates against ObjectNav goals/viewpoints only after visit order is fixed.",
            }
        )
        out.append(row)
    return out
...
def enrich_failure_rows(failure_rows: list[dict[str, Any]], source_gap_case_rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    case_by_scan = {str(row.get("scan_id")): row for row in source_gap_case_rows}
    out: list[dict[str, Any]] = []
    for row in failure_rows:
        case = case_by_scan.get(str(row.get("scan_id")), {})
        enriched = dict(row)
        enriched.update(
            {
                "version": VERSION,
                "source_gap_origin": True,
                "source_ready_after_m86_m87": bool(case.get("source_ready_after_m86_m87")),
                "claim_boundary": "M89 failure rows are proxy source-gap recovery diagnostics, not executed navigation failures.",
            }
        )
        out.append(enriched)
    return out
...
def build_claim_boundary_rows() -> list[dict[str, Any]]:
    return [
        {
            "version": VERSION,
            "claim_id": "supported_source_gap_leakage_safe_goal_eval_proxy",
            "supported": True,
            "claim_boundary": "M89 joins fixed source-gap visit-order rows to ObjectNav goals/viewpoints only as evaluation labels.",
        },
        {
            "version": VERSION,
            "claim_id": "unsupported_real_navigation_sr_spl",
            "supported": False,
            "claim_boundary": "M89 is not a Habitat trajectory execution and cannot claim real navigation SR/SPL.",
        },
        {
            "version": VERSION,
            "claim_id": "unsupported_final_rgbd_open_vocab_robustness",
            "supported": False,
            "claim_boundary": "M89 covers only two source-gap cases and does not provide heldout transfer or stronger navigation/search baselines.",
        },
        {
            "version": VERSION,
            "claim_id": "unsupported_human_intent_main_claim",
            "supported": False,
            "claim_boundary": "M89 detector policies are task-agnostic and do not test structured task context as a main effect.",
        },
    ]
...
def build_route_decision_rows(ready: bool, any_primary_success: bool) -> list[dict[str, Any]]:
    if ready:
        return [
            {
                "version": VERSION,
                "decision": "source_gap_goal_eval_smoke_ready_but_navigation_claim_blocked",
                "selected_next_unit": "E008-M90 source-gap detector-goal result interpretation and trajectory-execution decision",
                "reason": "M89 evaluates fixed source-gap visit-order rows against ObjectNav targets without policy leakage; next step is to interpret source-gap recovery and decide whether trajectory execution is justified.",
    ...
    return [
        {
            "version": VERSION,
            "decision": "repair_m89_source_gap_goal_evaluation_smoke",
            "selected_next_unit": "repair E008-M89 source-gap goal-evaluation smoke",
            "reason": "M89 rows are incomplete or use blocked eval-only fields as policy inputs.",
            "source_gap_proxy_recovery_observed": False,
            "launch_long_job_now": False,
            "real_navigation_sr_spl_ready": False,
            "final_real_rgbd_open_vocab_robustness_ready": False,
        }
...
def build_report(
    ...
    source_gap_case_goal_metric_rows: list[dict[str, Any]],
    ...
    case_lines = []
    for row in source_gap_case_goal_metric_rows:
        case_lines.append(
    ...
    return f"""# E008-M89 Source-Gap Detector Candidate Goal-Evaluation Smoke
...
- Source-gap case goal metric rows: {coverage['source_gap_case_goal_metric_rows']}.
...
- M89 uses `ObjectNav` goal/viewpoint fields only as evaluation labels.
- M89 reports `GoalEvalProxySR` / `GoalEvalProxySPL` style diagnostics, not real navigation `SR` / `SPL`.
- M89 covers only the two source-gap cases fixed by M83-M88.
- M89 does not make final real RGB-D/open-vocabulary robustness or human-intent main claims.
"""
...
def main() -> None:
    ...
    m88_coverage = read_json(M88_ARTIFACT_DIR / "coverage.json")
    ...
    nav_rows = read_jsonl(M87_ARTIFACT_DIR / "candidate_navmesh_validation_rows.jsonl")
    visit_rows = read_jsonl(M88_ARTIFACT_DIR / "candidate_visit_order_rows.jsonl")
    if not episode_rows:
        raise SystemExit("missing M64 val_mini_episode_rows.jsonl")
    if not source_gap_case_rows:
        raise SystemExit("missing M87 source_gap_case_source_ready_rows.jsonl")
    if not nav_rows:
        raise SystemExit("missing M87 candidate_navmesh_validation_rows.jsonl")
    if not visit_rows:
        raise SystemExit("missing M88 candidate_visit_order_rows.jsonl")

    source_gap_episode_ids = {str(row.get("adapter_episode_id")) for row in source_gap_case_rows}
    source_gap_episode_rows = [row for row in episode_rows if str(row.get("adapter_episode_id")) in source_gap_episode_ids]
    goal_rows = m70.build_full_val_mini_eval_goal_rows(source_gap_episode_rows)
    ...
    source_gap_case_goal_metric_rows = build_source_gap_case_goal_metric_rows(scan_metric_rows, source_gap_case_rows)
    failure_rows = enrich_failure_rows(m12.build_failure_rows(scan_metric_rows), source_gap_case_rows)
    ...
    ready = (
        bool(aggregate_rows)
        and len(goal_rows) == len(source_gap_case_rows)
        and len(source_gap_case_goal_metric_rows) == len(source_gap_case_rows) * len(aggregate_rows)
        and leakage_pass
    ...
    coverage = {
        ...
        if ready
        else "e008_m89_source_gap_detector_candidate_goal_evaluation_smoke_blocked",
        ...
        "source_gap_case_goal_metric_rows": len(source_gap_case_goal_metric_rows),
    ...
    for output_dir in (ARTIFACT_DIR, DATA_OUT_DIR):
        ...
        write_jsonl(output_dir / "source_gap_eval_goal_rows.jsonl", goal_rows)
        ...
        write_jsonl(output_dir / "source_gap_case_goal_metric_rows.jsonl", source_gap_case_goal_metric_rows)
    ...
    (ARTIFACT_DIR / "report.md").write_text(
        build_report(coverage, aggregate_rows, source_gap_case_goal_metric_rows, failure_rows),
        encoding="utf-8",
    )
