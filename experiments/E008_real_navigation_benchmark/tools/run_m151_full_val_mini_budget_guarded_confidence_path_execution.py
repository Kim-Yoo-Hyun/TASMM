#!/usr/bin/env python3
"""Execute E008-M150 budget-guarded confidence/path rows in Habitat."""
...
VERSION = "e008_m151_full_val_mini_budget_guarded_confidence_path_execution_v0"
READY_STATUS = "e008_m151_full_val_mini_budget_guarded_confidence_path_execution_ready"
BLOCKED_STATUS = "e008_m151_full_val_mini_budget_guarded_confidence_path_execution_blocked"
NEXT_UNIT = "E008-M152 full-val-mini budget-guarded confidence/path trajectory result interpretation / scale decision"

DEFAULT_M150_CONTRACT = (
    EXP_ROOT
    / "artifacts"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--m150-contract", default=str(DEFAULT_M150_CONTRACT))
...
def patch_m37_for_m151(m37: Any, contract: Path, out_root: Path, derived_out_root: Path) -> None:
    m37.VERSION = VERSION
...
def build_claim_boundary_rows(ready: bool) -> list[dict[str, Any]]:
    return [
        {
            "version": VERSION,
            "claim_id": "full_val_mini_budget_guarded_trajectory_execution",
            "supported": ready,
            "claim_boundary": "M151 executes the fixed M150 budget-guarded policy suite in Habitat.",
        },
        {
            "version": VERSION,
            "claim_id": "final_real_navigation_sr_spl",
            "supported": False,
            "claim_boundary": "M151 is an internal full-val-mini execution result; final navigation claim still requires M152 interpretation, heldout transfer, external navigation/search baselines, and failure analysis.",
        },
        {
            "version": VERSION,
            "claim_id": "final_real_rgbd_open_vocab_robustness",
            "supported": False,
            "claim_boundary": "M151 evaluates the current HM3D/ObjectNav detector-derived route and does not by itself establish final RGB-D/open-vocabulary robustness.",
        },
        {
            "version": VERSION,
            "claim_id": "human_intent_main_claim",
            "supported": False,
            "claim_boundary": "M151 target-free rows do not use human intent; E006-M08 remains the active human-intent boundary.",
        },
    ]
...
def build_route_decision_rows(ready: bool) -> list[dict[str, Any]]:
    return [
        {
            "version": VERSION,
            "decision": "interpret_full_val_mini_budget_guarded_trajectory_result"
            if ready
            else "repair_m151_runner_or_contract",
            "selected_next_unit": NEXT_UNIT
            if ready
            else "repair E008-M151 full-val-mini budget-guarded confidence/path execution",
            "launch_long_job_now": False,
            "real_navigation_sr_spl_ready": False,
            "full_val_mini_budget_guarded_trajectory_result_ready": ready,
        }
    ]
...
def build_report(
    ...
    return "\n".join(
        [
            "# E008-M151 Full-Val-Mini Budget-Guarded Confidence/Path Execution",
            "",
            ...
            f"- Full-val-mini budget-guarded trajectory result ready: {coverage['selected_next_unit'] == NEXT_UNIT}.",
            ...
            "## Claim Boundary",
            "",
            "- M151 is a full-val-mini execution result, not a final navigation benchmark claim by itself.",
            "- `ObjectNav` goal/viewpoints are used only after stops for metric computation.",
            "- Final `SR` / `SPL`, deployable policy, final RGB-D/open-vocabulary robustness, and human intent as a main claim remain blocked until M152+.",
            "",
        ]
    )
...
def main() -> None:
    args = parse_args()
    contract = Path(args.m150_contract)
    ...
    m37 = load_m37_module()
    patch_m37_for_m151(m37, contract, out_root, derived_out_root)
