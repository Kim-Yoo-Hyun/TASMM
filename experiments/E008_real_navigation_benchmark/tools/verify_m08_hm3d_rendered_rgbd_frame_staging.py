#!/usr/bin/env python3
"""Verify E008-M08 HM3D rendered RGB-D frame staging."""
...
VERSION = "e008_m08_hm3d_rendered_rgbd_frame_staging_verifier_v0"
...
def build_frame_rows(render_plan_rows: list[dict[str, Any]]) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    for plan in render_plan_rows:
        ...
        if not row["frame_ready"]:
            issues.append({k: row[k] for k in ["scan_id", "frame_id", "color_exists", "color_nonblank", "depth_exists", "depth_positive", "pose_valid", "info_ready"]})
    return rows, issues
...
def build_report(coverage: dict[str, Any], scan_rows: list[dict[str, Any]]) -> str:
    lines = [
        "# E008-M08 HM3D Rendered RGB-D Frame Staging Verification",
        "",
        "## Facts",
        "",
    ...
    lines.extend(
        [
            ...
            "## Claim Boundary",
            "",
            "- E008-M08 verifies frame staging only.",
            "- It does not run detector inference, H001 navigation execution, or `SR` / `SPL` evaluation.",
            "- Coordinate-frame and snap-to-navmesh validation remain required before policy execution.",
            "",
        ]
    )
    return "\n".join(lines)
...
def run() -> dict[str, Any]:
    ...
    coverage = {
        ...
        "status": "e008_m08_hm3d_rendered_rgbd_frame_staging_verified"
        if ready
        else "e008_m08_hm3d_rendered_rgbd_frame_staging_verification_failed",
...
def main() -> int:
    ...
    return 0 if coverage["status"] == "e008_m08_hm3d_rendered_rgbd_frame_staging_verified" else 2
