#!/usr/bin/env python3
"""Execute E008-M162 confidence-first constrained repair rows in Habitat."""
...
VERSION = "e008_m163_confidence_first_constrained_repair_trajectory_execution_v0"
READY_STATUS = "e008_m163_confidence_first_constrained_repair_trajectory_execution_ready"
BLOCKED_STATUS = "e008_m163_confidence_first_constrained_repair_trajectory_execution_blocked"
NEXT_UNIT = "E008-M164 confidence-first constrained repair trajectory result interpretation / protected-baseline gate"

DEFAULT_M162_CONTRACT = (
    EXP_ROOT / "artifacts" / "E008-M162_confidence_first_constrained_repair_trajectory_contract_v0"
)
DEFAULT_ARTIFACT_DIR = (
    EXP_ROOT / "artifacts" / "E008-M163_confidence_first_constrained_repair_trajectory_execution_v0"
)
...
BASELINE_POLICIES = [
    ...
    "confidence_first_no_path_tiebreak_v1",
    "source_gap_trigger_only_v1",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--m162-contract", default=str(DEFAULT_M162_CONTRACT))
...
def patch_m37_for_m163(m37: Any, contract: Path, out_root: Path, derived_out_root: Path) -> None:
    m37.VERSION = VERSION
    ...
    return [
        {
            ...
            "supported": ready,
            "claim_boundary": "M163 executes the fixed M162 confidence-first constrained repair policy suite in Habitat.",
        },
        {
            "version": VERSION,
            "claim_id": "confidence_first_repair_navigation_improvement",
            "supported": False,
            "claim_boundary": "Requires M164 protected-baseline interpretation before any positive navigation claim.",
        },
        {
            "version": VERSION,
            "claim_id": "final_real_navigation_sr_spl",
            "supported": False,
            "claim_boundary": "M163 is an internal full-val-mini execution result; final navigation claim still requires interpretation, heldout transfer, external navigation/search baselines, and failure analysis.",
        },
        {
            "version": VERSION,
            "claim_id": "human_intent_main_claim",
            "supported": False,
            "claim_boundary": "M163 target-free rows do not use human intent; E006-M08 remains the active human-intent boundary.",
        },
    ]
...
def build_route_decision_rows(ready: bool) -> list[dict[str, Any]]:
    return [
        {
            "version": VERSION,
            "decision": "interpret_confidence_first_repair_trajectory_result"
            if ready
            else "repair_m163_runner_or_contract",
            "selected_next_unit": NEXT_UNIT
            if ready
            else "repair E008-M163 confidence-first constrained repair trajectory execution",
            "launch_long_job_now": False,
            "real_navigation_sr_spl_ready": False,
            "confidence_first_repair_trajectory_result_ready": ready,
        }
    ]
...
def build_report(
    ...
    return "\n".join(
        [
            "# E008-M163 Confidence-First Constrained Repair Trajectory Execution",
            "",
            ...
            "## Claim Boundary",
            "",
            "- M163 is an execution result, not final interpretation.",
            "- `ObjectNav` goal/viewpoints are used only after stops for metric computation.",
            "- Final `SR` / `SPL`, deployable policy, final RGB-D/open-vocabulary robustness, and human intent as a main claim remain blocked until M164+.",
            "",
        ]
    )
...
def main() -> None:
    args = parse_args()
    contract = Path(args.m162_contract)
    ...
    m37 = load_m37_module()
    patch_m37_for_m163(m37, contract, out_root, derived_out_root)
