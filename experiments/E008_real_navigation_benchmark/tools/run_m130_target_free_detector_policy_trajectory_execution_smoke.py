#!/usr/bin/env python3
"""Execute E008-M129 target-free detector-policy rows as a Habitat trajectory smoke."""
...
VERSION = "e008_m130_target_free_detector_policy_trajectory_execution_smoke_v0"
READY_STATUS = "e008_m130_target_free_detector_policy_trajectory_execution_smoke_ready"
BLOCKED_STATUS = "e008_m130_target_free_detector_policy_trajectory_execution_smoke_blocked"
NEXT_UNIT = "E008-M131 target-free detector-policy trajectory result interpretation and scale decision"

DEFAULT_M129_CONTRACT = EXP_ROOT / "artifacts" / "E008-M129_target_free_detector_policy_trajectory_contract_v0"
DEFAULT_ARTIFACT_DIR = EXP_ROOT / "artifacts" / "E008-M130_target_free_detector_policy_trajectory_execution_smoke_v0"
...
def patch_m37_for_m130(m37: Any, contract: Path, out_root: Path, derived_out_root: Path) -> None:
    m37.VERSION = VERSION
...
def build_claim_boundary_rows(ready: bool) -> list[dict[str, Any]]:
    return [
        {
            "version": VERSION,
            "claim_id": "target_free_detector_policy_trajectory_smoke",
            "supported": ready,
            "claim_boundary": "M130 executes the M129 target-free detector-policy rows in Habitat as a one-case trajectory smoke.",
        },
        {
            "version": VERSION,
            "claim_id": "final_real_navigation_sr_spl",
            "supported": False,
            "claim_boundary": "M130 is still a one-case trajectory smoke; final navigation claim requires scale, heldout transfer, and stronger navigation/search baselines.",
        },
        {
            "version": VERSION,
            "claim_id": "final_real_rgbd_open_vocab_robustness",
            "supported": False,
            "claim_boundary": "M130 tests execution of one target-free detector route; it does not establish final RGB-D/open-vocabulary robustness.",
        },
        {
            "version": VERSION,
            "claim_id": "human_intent_main_claim",
            "supported": False,
            "claim_boundary": "M130 detector-policy rows do not use human intent; E006-M08 remains the active human-intent boundary.",
        },
    ]
...
def build_route_decision_rows(ready: bool) -> list[dict[str, Any]]:
    return [
        {
            "version": VERSION,
            "decision": "interpret_target_free_detector_policy_trajectory_result"
            if ready
            else "repair_m130_runner_or_contract",
            "selected_next_unit": NEXT_UNIT if ready else "repair E008-M130 target-free detector-policy trajectory execution smoke",
            "launch_long_job_now": False,
            "real_navigation_sr_spl_ready": False,
            "target_free_detector_policy_trajectory_result_ready": ready,
        }
    ]
...
def build_report(coverage: dict[str, Any], aggregate_rows: list[dict[str, Any]], pairwise_rows: list[dict[str, Any]]) -> str:
    ...
    return "\n".join(
        [
            "# E008-M130 Target-Free Detector-Policy Trajectory Execution Smoke",
            "",
            ...
            f"- Target-free trajectory result ready: {coverage['selected_next_unit'] == NEXT_UNIT}.",
            ...
            "## Claim Boundary",
            "",
            "- M130 is an executed one-case detector-policy trajectory smoke, not a final navigation benchmark.",
...
def main() -> None:
    args = parse_args()
    contract = Path(args.m129_contract)
    ...
    m37 = load_m37_module()
    patch_m37_for_m130(m37, contract, out_root, derived_out_root)
