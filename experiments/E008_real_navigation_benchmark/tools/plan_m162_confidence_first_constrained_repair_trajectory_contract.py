#!/usr/bin/env python3
"""Build E008-M162 confidence-first constrained repair trajectory contract."""
...
VERSION = "e008_m162_confidence_first_constrained_repair_trajectory_contract_v0"
READY_STATUS = "e008_m162_confidence_first_constrained_repair_trajectory_contract_ready_runner_next"
READY_RUNNER_MISSING_STATUS = (
    "e008_m162_confidence_first_constrained_repair_trajectory_contract_ready_runner_missing"
)
BLOCKED_STATUS = "e008_m162_confidence_first_constrained_repair_trajectory_contract_blocked"
...
ARTIFACT_DIR = (
    EXP_ROOT
    / "artifacts"
    / "E008-M162_confidence_first_constrained_repair_trajectory_contract_v0"
)
DATA_OUT_DIR = (
    ROOT
    / "local_dataset"
...
M163_RUNNER = EXP_ROOT / "tools" / "run_m163_confidence_first_constrained_repair_trajectory_execution.py"
M163_ARTIFACT_DIR = (
    EXP_ROOT
    / "artifacts"
    / "E008-M163_confidence_first_constrained_repair_trajectory_execution_v0"
)
M163_DATA_OUT_DIR = (
    ROOT
    / "local_dataset"
...
NEXT_UNIT = "E008-M163 confidence-first constrained repair trajectory execution"
...
def normalize_candidate_rows(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for row in rows:
        ...
        payload.update(
            {
                "version": VERSION,
                "m161_materialization_version": row.get("version"),
                "execution_contract_version": VERSION,
                "claim_boundary": "M162 fixes runner-compatible confidence-first constrained repair trajectory inputs only; no Habitat trajectory is executed.",
...
def build_plan_rows(
    m161_plan_rows: list[dict[str, Any]],
    ...
    for plan in m161_plan_rows:
        ...
        payload = {
            **plan,
            "version": VERSION,
            "m161_materialization_version": plan.get("version"),
            "execution_contract_version": VERSION,
            ...
            "runner_script": str(M163_RUNNER.relative_to(ROOT)),
            ...
            "repair_promotion_allowed_rows": audit.get("repair_promotion_allowed_rows"),
            "local_swap_promoted_rows": audit.get("local_swap_promoted_rows"),
            "local_swap_demoted_rows": audit.get("local_swap_demoted_rows"),
            "max_rank_displacement_abs_from_detector": audit.get("max_rank_displacement_abs_from_detector"),
            "m161_requires_cumulative_path_recompute_for_execution": plan.get(
            ...
            "policy_input_uses_eval_goal_or_viewpoint": False,
            "policy_input_uses_success_label": False,
            "claim_boundary": "M162 is a Docker trajectory contract/preflight unit; M163 is required for executed SR/SPL.",
        }
        out.append(payload)
    return out
...
def build_execution_contract_rows(
    ...
    for row in audit_rows:
        policy_id = str(row.get("policy_id"))
        promotion_sums[policy_id] += int(row.get("repair_promotion_allowed_rows") or 0)
        max_rank_disp[policy_id] = max(
            max_rank_disp[policy_id],
            int(row.get("max_rank_displacement_abs_from_detector") or 0),
        )
        ...
        rows.append(
            {
                ...
                "repair_promotion_allowed_rows": promotion_sums[policy_id],
                ...
                "execute_in_m163": True,
                "uses_objectnav_eval_goal_or_viewpoint_for_policy": False,
                "uses_objectnav_eval_goal_or_viewpoint_for_metric": True,
                "claim_boundary": "M162 fixes the confidence-first constrained repair execution contract only; M163 is required for executed SR/SPL.",
            }
        )
    return rows
...
def build_docker_preflight_rows(
    ...
    m163_compile: dict[str, Any] | None,
) -> list[dict[str, Any]]:
    ...
    return [
        ...
        {
            "version": VERSION,
            "check_id": "m163_confidence_first_runner_available",
            "status": "pass" if M163_RUNNER.exists() and m163_compile and m163_compile.get("ok") else "fail",
            "evidence": f"runner={M163_RUNNER.relative_to(ROOT)}; exists={M163_RUNNER.exists()}; py_compile={bool(m163_compile and m163_compile.get('ok'))}.",
        },
    ]


def build_m163_command_rows() -> list[dict[str, Any]]:
    command = (
        ...
        "experiments/E008_real_navigation_benchmark/tools/run_m163_confidence_first_constrained_repair_trajectory_execution.py "
    ...
    return [
        {
            "version": VERSION,
            "command_id": "e008_m163_confidence_first_constrained_repair_trajectory_execution",
            ...
            "runner_path": str(M163_RUNNER.relative_to(ROOT)),
            "runner_implemented": M163_RUNNER.exists(),
            "output_path": str(M163_ARTIFACT_DIR.relative_to(ROOT)),
            "derived_output_path": str(M163_DATA_OUT_DIR.relative_to(ROOT)),
            "command": command,
            "launch_now": False,
            "expected_files": [
                "coverage.json",
...
def build_readiness_gate_rows(
    *,
    ...
    m161_leakage_rows: list[dict[str, Any]],
    m161_audit_rows: list[dict[str, Any]],
    ...
    selected_audits = [row for row in m161_audit_rows if row.get("policy_id") == METHOD_POLICY]
    selected_changed = sum(1 for row in selected_audits if row.get("order_changed_vs_detector"))
    selected_promoted = sum(int(row.get("local_swap_promoted_rows") or 0) for row in selected_audits)
    selected_max_disp = max(
        [int(row.get("max_rank_displacement_abs_from_detector") or 0) for row in selected_audits],
        default=0,
    )
    gates = [
        (
            "m161_materialization_ready",
            m161_cov.get("status") == "e008_m161_confidence_first_constrained_repair_materialization_smoke_ready"
            and m161_cov.get("trajectory_contract_ready_next") is True,
            f"M161 status={m161_cov.get('status')}; trajectory_contract_ready_next={m161_cov.get('trajectory_contract_ready_next')}.",
            True,
        ),
        (
        ...
        (
            ...
            and selected_changed == int(m161_cov.get("selected_changed_episode_rows") or 0),
            f"selected_changed={selected_changed}; coverage={m161_cov.get('selected_changed_episode_rows')}.",
            True,
        ),
        (
            "selected_policy_local_swap_preserved",
            selected_promoted == int(m161_cov.get("selected_local_swap_promoted_rows") or 0),
            f"selected_promoted={selected_promoted}; coverage={m161_cov.get('selected_local_swap_promoted_rows')}.",
            True,
        ),
        (
            "rank_displacement_guard_preserved",
            selected_max_disp <= 1,
            f"selected_max_rank_displacement={selected_max_disp}.",
            True,
        ),
        (
            "m161_leakage_audit_pass",
            all(row.get("leakage_audit_pass") for row in m161_leakage_rows),
            f"failed={sum(1 for row in m161_leakage_rows if not row.get('leakage_audit_pass'))}.",
            True,
        ),
        (
            "m162_leakage_audit_pass",
        ...
        (
            ...
            False,
            "M162 is a contract/preflight unit; M163 should execute.",
            False,
        ),
    ]
    return [
        {
            ...
            "passed": passed,
            "blocks_m163": blocks and not passed,
            "evidence": evidence,
        }
...
def build_claim_boundary_rows() -> list[dict[str, Any]]:
    return [
        {
            "version": VERSION,
            "claim_id": "confidence_first_repair_execution_contract_ready",
            "status": "supported_contract_only",
            "safe_claim": "M162 provides a Docker-preflighted, runner-compatible confidence-first constrained repair trajectory execution contract.",
        },
        {
            "version": VERSION,
            "claim_id": "confidence_first_repair_trajectory_execution",
            "status": "not_executed",
            "safe_claim": "M162 does not execute Habitat trajectories; M163 is required for SR/SPL.",
        },
        {
            "version": VERSION,
            "claim_id": "positive_navigation_improvement",
            "status": "blocked",
            "safe_claim": "Positive navigation improvement needs M163 execution and M164 protected-baseline interpretation.",
        },
        {
            "version": VERSION,
            "claim_id": "final_real_navigation_sr_spl",
            "status": "blocked",
            "safe_claim": "Final real navigation claim needs execution, interpretation, heldout transfer, external navigation/search baselines, and failure analysis.",
        },
        {
            "version": VERSION,
            "claim_id": "human_intent_main_claim",
            "status": "blocked",
            "safe_claim": "M162 target-free execution contract does not use human intent; E006-M08 remains the active human-intent boundary.",
        },
    ]
...
def build_route_decision_rows(contract_ready: bool, runner_ready: bool) -> list[dict[str, Any]]:
    if contract_ready and runner_ready:
        ...
        decision = "run_m163_confidence_first_constrained_repair_trajectory_execution"
    elif contract_ready:
        selected_next = "E008-M163 confidence-first constrained repair trajectory execution runner scaffold"
        decision = "scaffold_m163_runner_before_docker_execution"
    else:
        selected_next = "repair E008-M162 confidence-first constrained repair trajectory contract"
        decision = "repair_m162_contract_or_preflight"
    return [
        {
            "version": VERSION,
            "row_type": "route_decision",
            "decision_id": "m162_selected_next",
            "decision": decision,
            "selected_next_unit": selected_next,
            "launch_long_job_now": False,
            "reason": "M162 fixes the confidence-first constrained repair execution input contract and Docker/data preflight; M163 should execute trajectories."
            if contract_ready
            else "One or more M162 contract/preflight gates failed.",
        }
    ]
...
def build_report(
    ...
    return "\n".join(
        [
            "# E008-M162 Confidence-First Constrained Repair Trajectory Contract",
            "",
            ...
            f"- Selected local-swap promoted rows: {coverage['selected_policy_local_swap_promoted_rows']}.",
            ...
            "## Gates",
            "",
            markdown_table(gate_rows, ["gate_id", "status", "blocks_m163", "evidence"]),
            "",
            "## Docker Preflight",
            "",
            ...
            "",
            "- M162 supports only the confidence-first constrained repair execution contract and Docker/data preflight.",
            "- M162 intentionally does not claim final real navigation `SR` / `SPL`, deployable search policy, or final real RGB-D/open-vocabulary robustness.",
            "- M163 must test whether the selected repair beats the protected detector-confidence baseline without increasing candidate visits.",
            "",
        ]
    )
...
def copy_support_files(output_dir: Path, candidate_rows: list[dict[str, Any]], plan_rows: list[dict[str, Any]]) -> None:
    ...
    for filename in M156_SUPPORT_FILES:
        src = M156_DIR / filename
        if src.exists():
            shutil.copy2(src, output_dir / filename)
    for filename in M161_AUX_FILES:
        src = M161_DIR / filename
        if src.exists():
            shutil.copy2(src, output_dir / f"m161_{filename}")
    write_jsonl(output_dir / "dynamic_stale_overlay_trajectory_candidate_rows.jsonl", candidate_rows)
    write_jsonl(output_dir / "confidence_first_candidate_rows.jsonl", candidate_rows)
    write_jsonl(output_dir / "trajectory_execution_plan_rows.jsonl", plan_rows)
    write_jsonl(output_dir / "confidence_first_execution_plan_rows.jsonl", plan_rows)
...
def main() -> None:
    ...
    m161_cov = read_json(M161_DIR / "coverage.json")
    base_candidate_rows = read_jsonl(M156_DIR / "base_candidate_rows.jsonl")
    raw_candidate_rows = read_jsonl(M161_DIR / "confidence_first_candidate_rows.jsonl")
    raw_plan_rows = read_jsonl(M161_DIR / "policy_plan_rows.jsonl")
    ...
    m161_audit_rows = read_jsonl(M161_DIR / "policy_order_audit_rows.jsonl")
    m161_leakage_rows = read_jsonl(M161_DIR / "leakage_audit_rows.jsonl")

    if not m161_cov:
        raise SystemExit("missing M161 coverage.json")
    if not all([base_candidate_rows, raw_candidate_rows, raw_plan_rows, goal_rows, oracle_rows, input_contract_rows]):
        raise SystemExit("missing one or more required M156/M161 rows for M162")

    candidate_rows = normalize_candidate_rows(raw_candidate_rows)
    plan_rows = build_plan_rows(raw_plan_rows, candidate_rows, m161_audit_rows)
    contract_rows = build_execution_contract_rows(plan_rows, candidate_rows, m161_audit_rows)
    ...
    nvidia_status = command_status(
        ["nvidia-smi", "--query-gpu=memory.free", "--format=csv,noheader,nounits"],
        timeout_s=10,
    )
    m37_compile = command_status(["python", "-m", "py_compile", str(M37_RUNNER)], timeout_s=30)
    m163_compile = (
        command_status(["python", "-m", "py_compile", str(M163_RUNNER)], timeout_s=30)
        if M163_RUNNER.exists()
        else None
    )
    ...
    gate_rows = build_readiness_gate_rows(
        ...
        m161_leakage_rows=m161_leakage_rows,
        m161_audit_rows=m161_audit_rows,
        leakage_rows=leakage_rows,
        docker_rows=docker_rows,
    )
    contract_ready = not any(row.get("blocks_m163") for row in gate_rows)
    runner_ready = M163_RUNNER.exists() and bool(m163_compile and m163_compile.get("ok"))
    route_rows = build_route_decision_rows(contract_ready, runner_ready)
    command_rows = build_m163_command_rows()
    ...
    coverage = {
        ...
        "m161_status": m161_cov.get("status"),
        ...
        "selected_policy_local_swap_promoted_rows": sum(int(row.get("local_swap_promoted_rows") or 0) for row in selected_audits),
    ...
    for output_dir in (ARTIFACT_DIR, DATA_OUT_DIR):
        ...
        write_jsonl(output_dir / "m163_command_rows.jsonl", command_rows)
