#!/usr/bin/env python3
"""Evaluate E008-M99 coverage-expansion detector visit-order rows against ObjectNav targets."""
...
ARTIFACT_DIR = EXP_ROOT / "artifacts" / "E008-M100_coverage_expansion_detector_candidate_goal_evaluation_smoke_v0"
DATA_OUT_DIR = (
    ROOT
    / "local_dataset"
    / "HM3D_navigation_bridge"
    / "E008-M100_coverage_expansion_detector_candidate_goal_evaluation_smoke_v0"
)
...
M99_ARTIFACT_DIR = EXP_ROOT / "artifacts" / "E008-M99_coverage_expansion_detector_candidate_visit_order_path_smoke_v0"
M93_DATA_DIR = (
    ROOT
    / "local_dataset"
    / "HM3D_navigation_bridge"
...
VERSION = "e008_m100_coverage_expansion_detector_candidate_goal_evaluation_smoke_v0"
...
def build_coverage_scan_goal_metric_rows(
    scan_metric_rows: list[dict[str, Any]],
    target_rows: list[dict[str, Any]],
    boundary_rows: list[dict[str, Any]],
) -> list[dict[str, Any]]:
    target_by_scan = {str(row.get("scan_id")): row for row in target_rows}
    boundary_by_scan = {str(row.get("scan_id")): row for row in boundary_rows}
    out: list[dict[str, Any]] = []
    for metric in scan_metric_rows:
        scan_id = str(metric.get("scan_id"))
        target = target_by_scan.get(scan_id, {})
        boundary = boundary_by_scan.get(scan_id, {})
        row = dict(metric)
        row.update(
            {
                "version": VERSION,
                "metric_scope": "coverage_expansion_scan_policy_goal_eval",
                "coverage_expansion_origin": True,
                ...
                "detector_prompt_enabled": bool(target.get("detector_prompt_enabled")),
                "evaluation_target_enabled_for_policy": bool(target.get("evaluation_target_enabled")),
                "hm3d_objectnav_category": target.get("hm3d_objectnav_category"),
                "uses_objectnav_eval_goal_or_viewpoint_for_metric": True,
                "real_navigation_sr_spl_ready": False,
                "claim_boundary": "M100 scores fixed M99 coverage-expansion visit-order rows against ObjectNav goals/viewpoints only after policy order is frozen.",
            }
        )
        out.append(row)
    return out
        ...
        enriched.update(
            {
                ...
                "source_ready_after_m98": bool(boundary.get("source_ready")),
                "hm3d_objectnav_category": target.get("hm3d_objectnav_category"),
                "claim_boundary": "M100 failure rows are leakage-safe proxy diagnostics, not executed navigation failures.",
            }
        )
        out.append(enriched)
    return out
...
def build_claim_boundary_rows() -> list[dict[str, Any]]:
    return [
        {
            "version": VERSION,
            "claim_id": "supported_coverage_expansion_leakage_safe_goal_eval_proxy",
            "supported": True,
            "claim_boundary": "M100 joins fixed M99 coverage-expansion visit rows to ObjectNav targets only as evaluation labels.",
        },
        {
            "version": VERSION,
            "claim_id": "unsupported_real_navigation_sr_spl",
            "supported": False,
            "claim_boundary": "M100 is not a Habitat trajectory execution and cannot claim real navigation SR/SPL.",
        },
        {
            "version": VERSION,
            "claim_id": "unsupported_final_rgbd_open_vocab_robustness",
            "supported": False,
            "claim_boundary": "M100 covers one coverage-expanded source-gap case and does not provide heldout transfer or external navigation/search baselines.",
        },
        {
            "version": VERSION,
            "claim_id": "unsupported_human_intent_main_claim",
            "supported": False,
            "claim_boundary": "M100 detector policies are task-agnostic and do not test structured task context as a main effect.",
        },
    ]
...
def build_route_decision_rows(ready: bool, any_primary_success: bool) -> list[dict[str, Any]]:
    if ready:
        return [
            {
                "version": VERSION,
                "decision": "coverage_expansion_goal_eval_smoke_ready_but_navigation_claim_blocked",
                "selected_next_unit": "E008-M101 coverage-expansion detector-goal result interpretation and trajectory-execution decision",
                "reason": "M100 evaluates frozen coverage-expansion detector visit-order rows against ObjectNav targets without policy leakage; next step is to interpret recovery and decide whether trajectory execution is justified.",
                "coverage_expansion_proxy_recovery_observed": any_primary_success,
    ...
    return [
        {
            "version": VERSION,
            "decision": "repair_m100_coverage_expansion_goal_evaluation_smoke",
            "selected_next_unit": "repair E008-M100 coverage-expansion goal-evaluation smoke",
            "reason": "M100 rows are incomplete or use blocked eval-only fields as policy inputs.",
            "coverage_expansion_proxy_recovery_observed": False,
            "launch_long_job_now": False,
            "real_navigation_sr_spl_ready": False,
            "final_real_rgbd_open_vocab_robustness_ready": False,
        }
...
def build_report(
    ...
    coverage_scan_goal_metric_rows: list[dict[str, Any]],
    failure_rows: list[dict[str, Any]],
) -> str:
    aggregate_lines = []
    ...
    scan_lines = []
    for row in coverage_scan_goal_metric_rows:
        scan_lines.append(
    ...
    return f"""# E008-M100 Coverage-Expansion Detector Candidate Goal-Evaluation Smoke
...
- Coverage scan goal metric rows: {coverage['coverage_scan_goal_metric_rows']}.
...
- Coverage-expansion proxy recovery observed: {coverage['coverage_expansion_proxy_recovery_observed']}.
...
- M100 uses `ObjectNav` goal/viewpoint fields only as evaluation labels.
- M100 reports `GoalEvalProxySR` / `GoalEvalProxySPL` style diagnostics, not real navigation `SR` / `SPL`.
- M100 covers the M93-M99 coverage-expanded case only.
- M100 does not make final real RGB-D/open-vocabulary robustness, deployable policy, or human-intent main claims.
"""
...
def main() -> None:
    ...
    m99_coverage = read_json(M99_ARTIFACT_DIR / "coverage.json")
    episode_rows = read_jsonl(M64_ARTIFACT_DIR / "val_mini_episode_rows.jsonl")
    target_rows = read_jsonl(M93_DATA_DIR / "coverage_expansion_object_target_rows.jsonl")
    boundary_rows = read_jsonl(M98_ARTIFACT_DIR / "scan_source_boundary_rows.jsonl")
    nav_rows = read_jsonl(M98_ARTIFACT_DIR / "candidate_navmesh_validation_rows.jsonl")
    visit_rows = read_jsonl(M99_ARTIFACT_DIR / "candidate_visit_order_rows.jsonl")
    if not episode_rows:
        raise SystemExit("missing M64 val_mini_episode_rows.jsonl")
    if not target_rows:
        raise SystemExit("missing M93 coverage_expansion_object_target_rows.jsonl")
    if not boundary_rows:
        raise SystemExit("missing M98 scan_source_boundary_rows.jsonl")
    if not nav_rows:
        raise SystemExit("missing M98 candidate_navmesh_validation_rows.jsonl")
    if not visit_rows:
        raise SystemExit("missing M99 candidate_visit_order_rows.jsonl")

    target_episode_ids = {str(row.get("adapter_episode_id")) for row in target_rows}
    ...
    coverage_scan_goal_metric_rows = build_coverage_scan_goal_metric_rows(scan_metric_rows, target_rows, boundary_rows)
    failure_rows = enrich_failure_rows(m12.build_failure_rows(scan_metric_rows), target_rows, boundary_rows)
    ...
    ready = (
        bool(aggregate_rows)
        and len(goal_rows) == len(target_rows)
        and len(coverage_scan_goal_metric_rows) == len(target_rows) * len(aggregate_rows)
        and leakage_pass
    ...
    coverage = {
        ...
        if ready
        else "e008_m100_coverage_expansion_detector_candidate_goal_evaluation_smoke_blocked",
        "generated_at": datetime.now().isoformat(timespec="seconds"),
        "m99_status": m99_coverage.get("status"),
        "artifact_output_root": str(ARTIFACT_DIR),
        "derived_output_root": str(DATA_OUT_DIR),
        "coverage_target_rows": len(target_rows),
        "eval_episode_rows": len(goal_rows),
        "expected_eval_episode_rows": len(target_rows),
        ...
        "coverage_scan_goal_metric_rows": len(coverage_scan_goal_metric_rows),
    ...
    for output_dir in (ARTIFACT_DIR, DATA_OUT_DIR):
        ...
        write_jsonl(output_dir / "coverage_expansion_eval_goal_rows.jsonl", goal_rows)
        ...
        write_jsonl(output_dir / "coverage_scan_goal_metric_rows.jsonl", coverage_scan_goal_metric_rows)
    ...
    (ARTIFACT_DIR / "report.md").write_text(
        build_report(coverage, aggregate_rows, coverage_scan_goal_metric_rows, failure_rows),
        encoding="utf-8",
    )
