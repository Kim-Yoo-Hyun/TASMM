#!/usr/bin/env python3
"""Verify E008-M85 source-gap render frame staging."""
...
ARTIFACT_DIR = EXP_ROOT / "artifacts" / "E008-M84_source_gap_non_oracle_source_observation_expansion_materialization_smoke_v0"
DATA_OUT_DIR = ROOT / "local_dataset" / "HM3D_navigation_bridge" / "E008-M84_source_gap_non_oracle_source_observation_expansion_materialization_smoke_v0"
M15_VERIFY_TOOL = EXP_ROOT / "tools" / "verify_m15_non_oracle_observation_expansion_frame_staging.py"
VERSION = "e008_m85_source_gap_render_frame_staging_verifier_v0"
...
def rewrite_status(old_status: str) -> str:
    if old_status == "e008_m15_non_oracle_observation_expansion_frame_staging_verified":
        return "e008_m85_source_gap_render_frame_staging_verified"
    if old_status == "e008_m15_non_oracle_observation_expansion_frame_staging_verified_with_snap_warnings":
        return "e008_m85_source_gap_render_frame_staging_verified_with_snap_warnings"
    return "e008_m85_source_gap_render_frame_staging_verification_failed"


def build_report(coverage: dict[str, Any]) -> str:
    return "\n".join(
        [
            "# E008-M85 Source-Gap Render Frame Staging Verification",
            "",
            "## Facts",
            "",
            ...
            "## Next",
            "",
            "- If verified, launch E008-M86 detector candidate-source generation on the staged source-gap frames.",
            "",
        ]
    )
...
def main() -> int:
    ...
    coverage.update(
        {
            ...
            "selected_next_unit": "E008-M86 full-val-mini source-gap detector candidate-source background launch"
            if ready
            else "repair E008-M85 source-gap render frame staging",
    ...
    write_json(args.artifact_dir / "m85_verification_coverage.json", coverage)
    write_text(args.artifact_dir / "m85_verification_report.md", build_report(coverage))
