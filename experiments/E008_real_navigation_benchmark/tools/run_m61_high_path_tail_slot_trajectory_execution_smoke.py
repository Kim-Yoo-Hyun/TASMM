#!/usr/bin/env python3
"""Execute E008-M60 high-path tail-slot rows as an E008-M61 trajectory smoke."""
...
VERSION = "e008_m61_high_path_tail_slot_trajectory_execution_smoke_v0"
READY_STATUS = "e008_m61_high_path_tail_slot_trajectory_execution_smoke_ready"
BLOCKED_STATUS = "e008_m61_high_path_tail_slot_trajectory_execution_smoke_blocked"
NEXT_UNIT = "E008-M62 high-path tail-slot trajectory result interpretation and scale decision"

DEFAULT_M60_CONTRACT = EXP_ROOT / "artifacts" / "E008-M60_high_path_tail_slot_trajectory_contract_v0"
DEFAULT_ARTIFACT_DIR = EXP_ROOT / "artifacts" / "E008-M61_high_path_tail_slot_trajectory_execution_smoke_v0"
...
def build_claim_boundary_rows(ready: bool) -> list[dict[str, Any]]:
    return [
        {
            "version": VERSION,
            "claim_id": "high_path_tail_slot_trajectory_smoke",
            "supported": ready,
            "claim_boundary": "M61 executes the M60 high-path tail-slot rows in Habitat as a controlled trajectory smoke.",
        },
        {
            "version": VERSION,
            "claim_id": "final_real_navigation_sr_spl",
            "supported": False,
            "claim_boundary": "M61 remains a small HM3D ObjectNav trajectory smoke; final navigation claim requires scale, heldout transfer, and stronger navigation/search baselines.",
        },
        {
            "version": VERSION,
            "claim_id": "human_intent_main_claim",
            "supported": False,
            "claim_boundary": "Structured task context is used as a memory-trust/re-observation condition, not as natural-language human-intent understanding.",
        },
    ]
...
def build_route_decision_rows(ready: bool) -> list[dict[str, Any]]:
    return [
        {
            "version": VERSION,
            "decision": "interpret_high_path_tail_slot_trajectory_result" if ready else "repair_m61_runner_or_contract",
            "selected_next_unit": NEXT_UNIT if ready else "repair E008-M61 high-path tail-slot trajectory execution smoke",
...
def build_report(coverage: dict[str, Any], aggregate_rows: list[dict[str, Any]], pairwise_rows: list[dict[str, Any]]) -> str:
    ...
    return "\n".join(
        [
            "# E008-M61 High-Path Tail-Slot Trajectory Execution Smoke",
            "",
            ...
            "## Claim Boundary",
            "",
            "- M61 is an executed trajectory smoke, not the final scaled navigation benchmark.",
            "- `ObjectNav` goal/viewpoints are used only after stops for metric computation.",
            "- The human-context claim remains conditional memory trust/re-observation, not natural-language intent.",
            "",
        ]
    )
...
def main() -> None:
    ...
    m37.DEFAULT_M36_CONTRACT = DEFAULT_M60_CONTRACT
