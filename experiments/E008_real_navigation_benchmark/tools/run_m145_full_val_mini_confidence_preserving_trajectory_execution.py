#!/usr/bin/env python3
"""Execute E008-M144 full-val-mini confidence-preserving rows in Habitat."""
...
VERSION = "e008_m145_full_val_mini_confidence_preserving_trajectory_execution_v0"
READY_STATUS = "e008_m145_full_val_mini_confidence_preserving_trajectory_execution_ready"
BLOCKED_STATUS = "e008_m145_full_val_mini_confidence_preserving_trajectory_execution_blocked"
NEXT_UNIT = "E008-M146 full-val-mini confidence-preserving trajectory result interpretation / scale decision"

DEFAULT_M144_CONTRACT = (
    EXP_ROOT / "artifacts" / "E008-M144_full_val_mini_confidence_preserving_trajectory_contract_v0"
)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--m144-contract", default=str(DEFAULT_M144_CONTRACT))
...
def patch_m37_for_m145(m37: Any, contract: Path, out_root: Path, derived_out_root: Path) -> None:
    m37.VERSION = VERSION
...
def build_claim_boundary_rows(ready: bool) -> list[dict[str, Any]]:
    return [
        {
            "version": VERSION,
            "claim_id": "full_val_mini_confidence_preserving_trajectory_execution",
            "supported": ready,
            "claim_boundary": "M145 executes the fixed M144 full-val-mini confidence-preserving policy suite in Habitat.",
        },
        {
            "version": VERSION,
            "claim_id": "final_real_navigation_sr_spl",
            "supported": False,
            "claim_boundary": "M145 is a first full-val-mini execution result; final navigation claim still requires M146 interpretation, heldout transfer, external navigation/search baselines, and failure analysis.",
        },
        {
            "version": VERSION,
            "claim_id": "final_real_rgbd_open_vocab_robustness",
            "supported": False,
            "claim_boundary": "M145 evaluates the current HM3D/ObjectNav detector-derived route and does not by itself establish final RGB-D/open-vocabulary robustness.",
        },
        {
            "version": VERSION,
            "claim_id": "human_intent_main_claim",
            "supported": False,
            "claim_boundary": "M145 target-free rows do not use human intent; E006-M08 remains the active human-intent boundary.",
        },
    ]
...
def build_route_decision_rows(ready: bool) -> list[dict[str, Any]]:
    return [
        {
            "version": VERSION,
            "decision": "interpret_full_val_mini_confidence_preserving_trajectory_result"
            if ready
            else "repair_m145_runner_or_contract",
            "selected_next_unit": NEXT_UNIT
            if ready
            else "repair E008-M145 full-val-mini confidence-preserving trajectory execution",
            "launch_long_job_now": False,
            "real_navigation_sr_spl_ready": False,
            "full_val_mini_confidence_preserving_trajectory_result_ready": ready,
        }
    ]
...
def build_report(
    ...
    return "\n".join(
        [
            "# E008-M145 Full-Val-Mini Confidence-Preserving Trajectory Execution",
            "",
            ...
            f"- Full-val-mini trajectory result ready: {coverage['selected_next_unit'] == NEXT_UNIT}.",
            ...
            "## Claim Boundary",
            "",
            "- M145 is a full-val-mini execution result, not a final navigation benchmark claim by itself.",
            "- `ObjectNav` goal/viewpoints are used only after stops for metric computation.",
            "- Final `SR` / `SPL`, deployable policy, final RGB-D/open-vocabulary robustness, and human intent as a main claim remain blocked until M146+.",
            "",
        ]
    )
...
def main() -> None:
    args = parse_args()
    contract = Path(args.m144_contract)
    ...
    m37 = load_m37_module()
    patch_m37_for_m145(m37, contract, out_root, derived_out_root)
