#!/usr/bin/env python3
"""Launch E008-M85 source-gap render frame staging in tmux."""
...
VERIFY_COMMAND = (
    ...
    f"--artifact-dir {M84_ARTIFACT_DIR} --data-out-dir {M84_DATA_DIR} --require-ready"
)
...
def count_existing_render_files() -> dict[str, int]:
    scan_root = M84_DATA_DIR / "3RScan" / "scans"
    return {
...
def build_render_command(prefix: list[str], log_path: Path) -> tuple[list[str], str, str]:
    render_input_dir = M84_DATA_DIR / "render_inputs"
    docker_command = [
        *prefix,
        "run",
        "--rm",
...
def build_preflight_rows(prefix: list[str], docker: dict[str, Any], render_plan_rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    m84 = read_json(M84_ARTIFACT_DIR / "coverage.json")
    image = command_status([*prefix, "image", "inspect", HABITAT_IMAGE, "--format", "{{.Id}}"]) if docker["available"] else {"available": False}
    return [
        {
            "gate_id": "m84_materialization_ready",
            "gate_status": "pass"
            if m84.get("status") == "e008_m84_source_gap_non_oracle_source_observation_expansion_materialization_smoke_ready"
            else "fail",
            "details": m84.get("status"),
        },
        {
            "gate_id": "render_plan_rows_ready",
            "gate_status": "pass" if len(render_plan_rows) == 192 else "fail",
            "details": len(render_plan_rows),
        },
        {
            "gate_id": "render_script_ready",
            "gate_status": "pass" if (M84_DATA_DIR / "render_inputs" / "render_m84.py").exists() else "fail",
            "details": str(M84_DATA_DIR / "render_inputs" / "render_m84.py"),
        },
        {
...
def build_report(coverage: dict[str, Any], preflight_rows: list[dict[str, Any]]) -> str:
    return "\n".join(
        [
            "# E008-M85 Source-Gap Render Frame Staging Launch",
            "",
            "## Facts",
            "",
            ...
            *[
            ...
            "## Claim Boundary",
            "",
            "- M85 is a background render-frame staging launch only.",
            "- M85 does not support detector candidate quality, source-gap recovery, real navigation `SR` / `SPL`, or final RGB-D/open-vocabulary robustness.",
            "",
            "## Next",
            "",
            "- Verify E008-M85 completion before launching E008-M86 detector candidate-source generation.",
            "",
        ]
    )
...
def main() -> int:
    ...
    render_plan_rows = read_jsonl(M84_DATA_DIR / "render_inputs" / "render_plan_rows.jsonl")
    ...
    log_path = LOG_DIR / f"{timestamp}_e008_m85_source_gap_render.log"
    ...
    status = "e008_m85_source_gap_render_frame_staging_launch_blocked"
    if not blockers:
        launch_result = command_status(tmux_command)
        launch_executed = bool(launch_result["available"])
        status = (
            "e008_m85_source_gap_render_frame_staging_launched"
            if launch_executed
            else "e008_m85_source_gap_render_frame_staging_launch_failed"
        )
    elif tmux_running(TMUX_SESSION) and blockers == ["tmux_session_free"]:
        status = "e008_m85_source_gap_render_frame_staging_already_running"
    ...
    coverage = {
        ...
        "selected_next_unit": "E008-M85 completion verification",
    }
    command_rows = [
        {
            "job_id": "E008-M85",
