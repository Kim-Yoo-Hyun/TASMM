#!/usr/bin/env python3
"""Execute E008-M139 confidence-preserving repair rows as a Habitat trajectory smoke."""
...
VERSION = "e008_m140_target_free_confidence_preserving_repair_trajectory_execution_smoke_v0"
READY_STATUS = "e008_m140_target_free_confidence_preserving_repair_trajectory_execution_smoke_ready"
BLOCKED_STATUS = "e008_m140_target_free_confidence_preserving_repair_trajectory_execution_smoke_blocked"
NEXT_UNIT = "E008-M141 target-free confidence-preserving repair trajectory result interpretation / scale decision"

DEFAULT_M139_CONTRACT = (
    EXP_ROOT / "artifacts" / "E008-M139_target_free_confidence_preserving_repair_trajectory_contract_v0"
)
...
BASELINE_POLICIES = [
    ...
    "confidence_preserving_hard_veto_v0",
    "trajectory_greedy_confidence_path_repair_v0",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--m139-contract", default=str(DEFAULT_M139_CONTRACT))
...
def patch_m37_for_m140(m37: Any, contract: Path, out_root: Path, derived_out_root: Path) -> None:
    m37.VERSION = VERSION
...
def build_claim_boundary_rows(ready: bool) -> list[dict[str, Any]]:
    return [
        {
            "version": VERSION,
            "claim_id": "target_free_confidence_preserving_repair_trajectory_smoke",
            "supported": ready,
            "claim_boundary": "M140 executes the M139 confidence-preserving target-free repair rows in Habitat as a one-case trajectory smoke.",
        },
        {
            "version": VERSION,
            "claim_id": "final_real_navigation_sr_spl",
            "supported": False,
            "claim_boundary": "M140 is still a one-case trajectory smoke; final navigation claim requires scale, heldout transfer, and stronger navigation/search baselines.",
        },
        {
            "version": VERSION,
            "claim_id": "final_real_rgbd_open_vocab_robustness",
            "supported": False,
            "claim_boundary": "M140 tests execution of one target-free confidence-preserving route; it does not establish final RGB-D/open-vocabulary robustness.",
        },
        {
            "version": VERSION,
            "claim_id": "human_intent_main_claim",
            "supported": False,
            "claim_boundary": "M140 target-free rows do not use human intent; E006-M08 remains the active human-intent boundary.",
        },
    ]
...
def build_route_decision_rows(ready: bool) -> list[dict[str, Any]]:
    return [
        {
            "version": VERSION,
            "decision": "interpret_target_free_confidence_preserving_repair_trajectory_result"
            if ready
            else "repair_m140_runner_or_contract",
            "selected_next_unit": NEXT_UNIT
            if ready
            else "repair E008-M140 target-free confidence-preserving repair trajectory execution smoke",
            "launch_long_job_now": False,
            "real_navigation_sr_spl_ready": False,
            "target_free_confidence_preserving_repair_result_ready": ready,
        }
    ]
...
def build_report(coverage: dict[str, Any], aggregate_rows: list[dict[str, Any]], pairwise_rows: list[dict[str, Any]]) -> str:
    ...
    return "\n".join(
        [
            "# E008-M140 Target-Free Confidence-Preserving Repair Trajectory Execution Smoke",
            "",
            ...
            f"- Confidence-preserving repair trajectory result ready: {coverage['selected_next_unit'] == NEXT_UNIT}.",
            ...
            "",
            "## Confidence-Preserving Policy Pairwise Delta Summary",
            "",
            m37.markdown_table(
                pairwise_summary,
                [
            ...
            "## Claim Boundary",
            "",
            "- M140 is an executed one-case confidence-preserving repair smoke, not a final navigation benchmark.",
...
def main() -> None:
    args = parse_args()
    contract = Path(args.m139_contract)
    ...
    m37 = load_m37_module()
    patch_m37_for_m140(m37, contract, out_root, derived_out_root)
