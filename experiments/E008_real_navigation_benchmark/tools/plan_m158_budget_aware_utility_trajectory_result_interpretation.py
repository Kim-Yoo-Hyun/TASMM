#!/usr/bin/env python3
"""Interpret E008-M157 budget-aware utility trajectory results."""
...
VERSION = "e008_m158_budget_aware_utility_trajectory_result_interpretation_v0"
READY_STATUS = "e008_m158_budget_aware_utility_trajectory_result_interpretation_ready"
BLOCKED_STATUS = "e008_m158_budget_aware_utility_trajectory_result_interpretation_blocked"
NEXT_UNIT = "E008-M159 budget-aware utility component failure decomposition / next-route decision"
...
def policy_role(policy_id: str) -> str:
    return {
        METHOD_POLICY: "selected_budget_aware_utility_method",
        PRIMARY_BASELINE: "protected_detector_confidence_baseline",
        NO_PATH_GAIN_ABLATION: "path_gain_component_ablation",
        NO_SOURCE_GAP_ABLATION: "source_gap_bonus_component_ablation",
        NO_VISIT_PENALTY_ABLATION: "visit_penalty_component_ablation",
        NO_VISIT_GUARD_ABLATION: "visit_guard_tradeoff_reference",
...
def interpret_policy(
    ...
    if policy_id == METHOD_POLICY:
        if (sr_delta or 0.0) >= 0.0 and (spl_delta or 0.0) < 0.0 and (visit_delta or 0.0) > 0.0:
            return "selected_utility_ties_sr_but_loses_spl_and_visit_efficiency_to_detector_confidence"
        if (spl_delta or 0.0) < 0.0:
            return "selected_utility_loses_spl_to_detector_confidence"
        return "selected_utility_requires_manual_review"
    if policy_id == PRIMARY_BASELINE:
        return "protected_detector_confidence_baseline_dominates_or_matches_selected_utility"
    if policy_id == NO_PATH_GAIN_ABLATION:
        return "removing_path_gain_matches_detector_confidence_and_improves_over_selected_utility"
    if policy_id == NO_SOURCE_GAP_ABLATION:
        return "removing_source_gap_bonus_is_equivalent_to_selected_utility_on_this_denominator"
    if policy_id == NO_VISIT_PENALTY_ABLATION:
        return "removing_visit_penalty_is_equivalent_to_selected_utility_on_this_denominator"
    if policy_id == NO_VISIT_GUARD_ABLATION:
        return (
            "no_visit_guard_has_best_spl_but_spends_more_candidate_visits_than_selected_utility"
            if is_best_spl
        ...
        rows.append(
            {
                ...
                "interpretation": interpret_policy(policy_id, sr_delta, spl_delta, path_delta, visit_delta, is_best_spl),
            }
        )
    return rows
...
def interpret_pairwise_summary(baseline_id: str, spl_delta: float | None, visit_delta: float | None) -> str:
    if baseline_id in {PRIMARY_BASELINE, NO_PATH_GAIN_ABLATION}:
        if (spl_delta or 0.0) < 0.0 and (visit_delta or 0.0) > 0.0:
            return "selected_utility_loses_mean_spl_and_visit_efficiency_to_detector_confidence_or_no_path_gain"
        return "selected_utility_does_not_show_required_gain_over_protected_confidence_family"
    if baseline_id in {NO_SOURCE_GAP_ABLATION, NO_VISIT_PENALTY_ABLATION}:
        return "component_ablation_is_equivalent_to_selected_utility; component_has_no_observed_effect"
    if baseline_id == NO_VISIT_GUARD_ABLATION:
        return "no_visit_guard_beats_selected_on_spl_but_spends_more_visits; budget_tradeoff_unresolved"
    if baseline_id == NO_CONFIDENCE_FLOOR_ABLATION:
        return "selected_utility_beats_no_confidence_floor_negative_control; confidence_floor_needed"
    return "manual_review"
...
def build_episode_delta_profile_rows(scan_rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    return [
        compare_episode_profile(
            grouped,
            PRIMARY_BASELINE,
            "selected_utility_has_small_mixed_episode_changes_vs_detector_confidence_with_negative_aggregate_spl_and_visits",
        ),
        compare_episode_profile(
            grouped,
            NO_PATH_GAIN_ABLATION,
            "path_gain_ablation_matches_detector_confidence_and_is_better_on_aggregate_than_selected_utility",
        ),
        compare_episode_profile(
            grouped,
            NO_SOURCE_GAP_ABLATION,
            "source_gap_bonus_ablation_is_equivalent_to_selected_utility",
        ),
        compare_episode_profile(
            grouped,
            NO_VISIT_PENALTY_ABLATION,
            "visit_penalty_ablation_is_equivalent_to_selected_utility",
        ),
        compare_episode_profile(
            grouped,
            NO_VISIT_GUARD_ABLATION,
            "no_visit_guard_has_spl_advantage_but_is_visit_budget_expensive",
        ),
        compare_episode_profile(
            grouped,
            NO_CONFIDENCE_FLOOR_ABLATION,
            "selected_utility_recovers_the_no_confidence_floor_negative_control",
        ),
    ]
...
def build_gate_rows(
    ...
    no_path_summary = next(row for row in pairwise_rows if row["baseline_policy_id"] == NO_PATH_GAIN_ABLATION)
    no_source_summary = next(row for row in pairwise_rows if row["baseline_policy_id"] == NO_SOURCE_GAP_ABLATION)
    no_visit_penalty_summary = next(row for row in pairwise_rows if row["baseline_policy_id"] == NO_VISIT_PENALTY_ABLATION)
    ...
    return [
        gate(
            "m157_input_ready",
            "pass" if not missing_inputs and m157_coverage.get("status") == "e008_m157_budget_aware_utility_trajectory_execution_ready" else "fail",
            "M157 full-val-mini budget-aware utility execution artifact is present and ready.",
            blocks_final=True,
        ),
        gate(
            "leakage_audit_pass",
            "pass" if bool(m157_coverage.get("leakage_audit_pass")) else "fail",
            "ObjectNav goal/viewpoint fields are metric-only and not policy inputs.",
            blocks_final=True,
        ),
        gate(
            "denominator_execution",
            "pass" if m157_coverage.get("scan_task_policy_rows") == 210 else "fail",
            "M157 executed the frozen 30 episode x 7 policy full-val-mini suite.",
            blocks_final=True,
        ),
        gate(
            "protected_baseline_sr",
            "pass" if (finite_float(method.get("delta_SR_vs_detector_confidence")) or 0.0) >= 0.0 else "fail",
            "Selected utility ties detector-confidence SR.",
            blocks_final=True,
        ),
        gate(
            "protected_baseline_spl",
            "fail" if (finite_float(method.get("delta_SPL_vs_detector_confidence")) or 0.0) < 0.0 else "pass",
            "Selected utility loses mean SPL to the protected detector-confidence baseline.",
            blocks_final=True,
        ),
        gate(
            "visit_efficiency",
            "fail" if (finite_float(method.get("delta_CandidateVisits_mean_vs_detector_confidence")) or 0.0) > 0.0 else "pass",
            "Selected utility visits more candidates on average than detector-confidence.",
            blocks_final=True,
        ),
        gate(
            "path_gain_component_value",
            "fail" if (finite_float(no_path_summary.get("delta_SPL_mean")) or 0.0) < 0.0 else "warning",
            "Removing path gain improves or matches SPL/visits, so the path-gain term is not supported in its current form.",
            blocks_final=True,
        ),
        gate(
            "source_gap_bonus_component_value",
            "fail" if abs(finite_float(no_source_summary.get("delta_SPL_mean")) or 0.0) <= 1e-9 else "warning",
            "Removing the source-gap bonus is equivalent to the selected policy on this denominator.",
            blocks_final=True,
        ),
        gate(
            "visit_penalty_component_value",
            "fail" if abs(finite_float(no_visit_penalty_summary.get("delta_SPL_mean")) or 0.0) <= 1e-9 else "warning",
            "Removing the visit penalty is equivalent to the selected policy on this denominator.",
            blocks_final=True,
        ),
        gate(
        ...
        gate(
            ...
            "Selected utility beats the no-confidence-floor negative control, so confidence remains a necessary guard.",
            blocks_final=False,
        ),
        gate(
            "visit_guard_tradeoff",
            "warning" if bool(no_visit_guard.get("is_best_spl_policy")) else "pass",
            "No-visit-guard ablation has best SPL but higher candidate visits, so utility must handle budget/SPL Pareto tradeoff explicitly.",
            blocks_final=True,
        ),
        gate(
            "episode_level_consistency",
            "warning",
            f"Selected utility beats detector SPL in {detector_summary.get('method_better_spl_rows')} / {detector_summary.get('rows')} rows, loses in {detector_summary.get('method_worse_spl_rows')} rows, and has more-visit rows {detector_summary.get('method_more_visit_rows')}.",
            blocks_final=True,
        ),
        gate(
        ...
        gate(
            ...
            "pass",
            "M157 is useful as a budget-aware utility component diagnostic execution table.",
            blocks_final=False,
        ),
        gate(
...
def build_claim_boundary_rows() -> list[dict[str, Any]]:
    return [
        claim(
            "full_val_mini_budget_aware_utility_execution_table",
            True,
            "M157 executes the fixed 30-episode, 7-policy budget-aware utility trajectory suite in Docker Habitat.",
        ),
        claim(
            "selected_budget_aware_utility_positive_navigation_improvement",
            False,
            "Selected utility ties SR but loses mean SPL and candidate-visit efficiency to detector-confidence.",
        ),
        claim(
            "path_gain_component_supported",
            False,
            "Removing path gain matches detector-confidence and is better than the selected utility on aggregate.",
        ),
        claim(
            "source_gap_bonus_component_supported",
            False,
            "Removing source-gap bonus is equivalent to the selected utility on this denominator.",
        ),
        claim(
            "visit_penalty_component_supported",
            False,
            "Removing visit penalty is equivalent to the selected utility on this denominator.",
        ),
        claim(
        ...
        claim(
            ...
            False,
            "M157 target-free rows do not use human intent; E006-M08 remains the active boundary.",
        ),
    ]
...
def build_reviewer_defense_rows() -> list[dict[str, Any]]:
    return [
        defense(
            "does_m157_prove_navigation_improvement",
            "No. It proves budget-aware utility execution works, but the selected utility loses mean SPL and visit efficiency to detector-confidence.",
        ),
        defense(
            "why_not_claim_utility_is_better",
            "The predefined protected-baseline gate fails, and removing path gain improves the selected policy. The utility form needs component-level repair.",
        ),
        defense(
            "what_component_survives",
            "The confidence floor survives: removing it sharply reduces SPL and increases candidate visits. Path gain, source-gap bonus, and visit penalty are not supported in their current form.",
        ),
        defense(
            "why_not_pick_no_visit_guard_posthoc",
            "`budget_guarded_no_visit_guard_v1` has the best SPL but spends more visits, so it is a Pareto tradeoff witness rather than a deployable selected method.",
        ),
        defense(
            "is_this_hypothesis_fitting",
            "No. The negative result is kept as a failure diagnosis and routes to component failure decomposition instead of changing thresholds or denominators.",
        ),
    ]
...
def build_route_decision_rows() -> list[dict[str, Any]]:
    return [
        {
            "version": VERSION,
            "route_id": "claim_selected_budget_aware_navigation_improvement",
            "decision": "reject_now",
            "selected": False,
            "selected_next_unit": None,
            "reason": "Selected utility loses mean SPL and visit efficiency to detector-confidence on full-val-mini.",
            "launch_long_job_now": False,
        },
        {
            "version": VERSION,
            "route_id": "scale_selected_budget_aware_policy",
            "decision": "reject_now",
            "selected": False,
            "selected_next_unit": None,
            "reason": "Scaling a policy that fails protected-baseline and component-value gates would violate novelty discipline.",
            "launch_long_job_now": False,
        },
        {
            "version": VERSION,
            "route_id": "record_m157_as_utility_diagnostic_table",
            "decision": "select",
            "selected": True,
            "selected_next_unit": None,
            "reason": "M157 is useful as a budget-aware utility component diagnostic execution table.",
            "launch_long_job_now": False,
        },
        {
            "version": VERSION,
            "route_id": "utility_component_failure_decomposition",
            "decision": "select_next",
            "selected": True,
            "selected_next_unit": NEXT_UNIT,
            "reason": "M159 should separate harmful path gain, inert source-gap/visit-penalty terms, confidence-floor necessity, and no-visit-guard Pareto tradeoff.",
            "launch_long_job_now": False,
        },
        {
            "version": VERSION,
            ...
            "selected": False,
            "selected_next_unit": None,
            "reason": "External navigation baselines remain required, but the internal utility failure mechanism should be documented before another heavy integration.",
            "launch_long_job_now": False,
        },
    ]
...
def build_report(
    ...
    return "\n".join(
        [
            "# E008-M158 Budget-Aware Utility Result Interpretation",
            "",
            ...
            f"- M157 status: `{coverage['m157_status']}`.",
            ...
            "## Claim Boundary",
            "",
            "- M158 rejects a positive navigation-improvement claim for the selected budget-aware utility policy.",
            "- M157 remains a full-val-mini diagnostic execution table.",
            "- M159 should decompose why path gain is harmful, source-gap/visit-penalty terms are inert, and confidence floor remains necessary.",
            "",
        ]
    )
...
def main() -> None:
    m156_coverage = read_json(M156_DIR / "coverage.json")
    m157_coverage = read_json(M157_DIR / "coverage.json")
    metric_rows = read_jsonl(M157_DIR / "dynamic_stale_trajectory_policy_metric_rows.jsonl")
    pairwise_raw = read_jsonl(M157_DIR / "pairwise_policy_delta_rows.jsonl")
    scan_rows = scan_metric_rows(metric_rows)

    required_inputs = [
        M156_DIR / "coverage.json",
        M157_DIR / "coverage.json",
        M157_DIR / "dynamic_stale_trajectory_policy_metric_rows.jsonl",
        M157_DIR / "pairwise_policy_delta_rows.jsonl",
        M157_DIR / "claim_boundary_rows.jsonl",
    ]
    ...
    gate_rows = build_gate_rows(m157_coverage, policy_rows, pairwise_rows, missing_inputs)
    ...
    primary = metrics.get(PRIMARY_BASELINE, {})
    no_path = metrics.get(NO_PATH_GAIN_ABLATION, {})
    no_source = metrics.get(NO_SOURCE_GAP_ABLATION, {})
    no_visit_penalty = metrics.get(NO_VISIT_PENALTY_ABLATION, {})
    no_visit_guard = metrics.get(NO_VISIT_GUARD_ABLATION, {})
    no_conf_floor = metrics.get(NO_CONFIDENCE_FLOOR_ABLATION, {})
    best_policy = max(
        policy_rows,
    ...
    coverage = {
        ...
        "m157_scan_task_policy_rows": m157_coverage.get("scan_task_policy_rows"),
        "m157_trajectory_attempt_rows": m157_coverage.get("trajectory_attempt_rows"),
        "m157_leakage_audit_pass": m157_coverage.get("leakage_audit_pass"),
        "method_policy_id": METHOD_POLICY,
        ...
        "without_path_gain_CandidateVisits_mean": no_path.get("CandidateVisits_mean"),
        ...
        "component_failure_decomposition_ready": not missing_blocked,
