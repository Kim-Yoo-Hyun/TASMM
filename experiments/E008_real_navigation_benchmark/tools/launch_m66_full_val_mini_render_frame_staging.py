#!/usr/bin/env python3
"""Launch E008-M66 full-val-mini render frame staging in tmux."""
...
ARTIFACT_DIR = EXP_ROOT / "artifacts" / "E008-M66_full_val_mini_render_frame_staging_launch_v0"
LOG_DIR = ROOT / "logs"
...
TMUX_SESSION = "e008_m66_full_val_mini_render"
VERSION = "e008_m66_full_val_mini_render_frame_staging_launch_v0"
...
def count_existing_render_files() -> dict[str, int]:
    scan_root = M65_DATA_DIR / "3RScan" / "scans"
    return {
...
def build_preflight_rows(prefix: list[str], docker: dict[str, Any], render_plan_rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    m65 = read_json(M65_ARTIFACT_DIR / "coverage.json")
    image = command_status([*prefix, "image", "inspect", HABITAT_IMAGE, "--format", "{{.Id}}"]) if docker["available"] else {"available": False}
    return [
        {
            "gate_id": "m65_contract_ready",
            "gate_status": "pass" if m65.get("status") == "e008_m65_full_val_mini_render_detector_contract_ready" else "fail",
            "details": m65.get("status"),
        },
        {
            "gate_id": "render_plan_rows_ready",
            "gate_status": "pass" if len(render_plan_rows) == 1080 else "fail",
            "details": len(render_plan_rows),
        },
        {
            "gate_id": "render_script_ready",
            "gate_status": "pass" if (M65_DATA_DIR / "render_inputs" / "render_m65.py").exists() else "fail",
            "details": str(M65_DATA_DIR / "render_inputs" / "render_m65.py"),
        },
        {
...
def build_report(coverage: dict[str, Any], preflight_rows: list[dict[str, Any]]) -> str:
    return "\n".join(
        [
            "# E008-M66 Full-Val-Mini Render Frame Staging Launch",
            "",
            "## 사실",
            "",
            ...
            *[
            ...
            "## 논문 주장",
            "",
            "- M66 launch does not support real navigation `SR` / `SPL` or detector robustness claims.",
            "- A verified M66 result only means RGB-D/pose frames are staged for the later detector candidate-source run.",
            "",
            "## 에이전트 추론",
            "",
            "- The next dependent task is M66 completion verification; M67 detector launch should wait until this verification passes.",
            "",
        ]
    )
...
def main() -> int:
    ...
    render_plan_rows = read_jsonl(M65_DATA_DIR / "render_inputs" / "render_plan_rows.jsonl")
    ...
    log_path = LOG_DIR / f"{timestamp}_e008_m66_full_val_mini_render.log"
    ...
    status = "e008_m66_full_val_mini_render_frame_staging_launch_blocked"
    if not blockers:
        launch_result = command_status(tmux_command)
        launch_executed = bool(launch_result["available"])
        status = (
            "e008_m66_full_val_mini_render_frame_staging_launched"
            if launch_executed
            else "e008_m66_full_val_mini_render_frame_staging_launch_failed"
        )
    elif tmux_running(TMUX_SESSION) and blockers == ["tmux_session_free"]:
        status = "e008_m66_full_val_mini_render_frame_staging_already_running"
    ...
    coverage = {
        ...
        "selected_next_unit": "E008-M66 completion verification",
    }
    command_rows = [
        {
            "job_id": "E008-M66",
