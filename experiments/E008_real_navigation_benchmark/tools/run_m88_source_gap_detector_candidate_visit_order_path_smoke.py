#!/usr/bin/env python3
"""Materialize source-gap detector candidate visit-order/path rows after E008-M87."""
...
ARTIFACT_DIR = EXP_ROOT / "artifacts" / "E008-M88_source_gap_detector_candidate_visit_order_path_smoke_v0"
DATA_OUT_DIR = ROOT / "local_dataset" / "HM3D_navigation_bridge" / "E008-M88_source_gap_detector_candidate_visit_order_path_smoke_v0"
M87_ARTIFACT_DIR = EXP_ROOT / "artifacts" / "E008-M87_source_gap_detector_candidate_navmesh_validation_v0"
M11_TOOL = EXP_ROOT / "tools" / "run_m11_detector_candidate_visit_order_path_smoke.py"
VERSION = "e008_m88_source_gap_detector_candidate_visit_order_path_smoke_v0"
...
def build_source_gap_case_policy_rows(
    scan_metric_rows: list[dict[str, Any]],
    source_gap_case_rows: list[dict[str, Any]],
) -> list[dict[str, Any]]:
    case_by_scan = {str(row.get("scan_id")): row for row in source_gap_case_rows}
    out: list[dict[str, Any]] = []
    for metric in scan_metric_rows:
        case = case_by_scan.get(str(metric.get("scan_id")))
        if not case:
            continue
        out.append(
            {
                "version": VERSION,
                "metric_scope": "source_gap_case_policy",
                ...
                "object_category": case.get("object_category") or metric.get("object_category"),
                "source_gap_origin": True,
                "source_gap_resolved_before_m84": bool(case.get("source_gap_resolved_before_m84")),
                "source_ready_after_m86_m87": bool(case.get("source_ready_after_m86_m87")),
                ...
                and bool(case.get("source_ready_after_m86_m87")),
                "uses_objectnav_eval_goal_or_viewpoint_for_policy": metric.get(
                    "uses_objectnav_eval_goal_or_viewpoint_for_policy"
                ),
                "real_navigation_sr_spl_ready": False,
                "claim_boundary": "M88 evaluates source-gap candidate visit order and source-to-candidate path costs only; it does not score goal hits or execute trajectories.",
            }
        )
    return out
...
def build_claim_boundary_rows() -> list[dict[str, Any]]:
    return [
        {
            "version": VERSION,
            "claim_id": "supported_source_gap_visit_order_path_materialization",
            "supported": True,
            "claim_boundary": "M88 materializes source-gap detector candidate visit-order and source-to-candidate path-cost rows without eval-goal/viewpoint policy leakage.",
        },
        {
            "version": VERSION,
            "claim_id": "unsupported_source_gap_recovery",
            "supported": False,
            "claim_boundary": "M88 does not compare candidates against eval-only ObjectNav goals or viewpoints, so it cannot claim target recovery.",
        },
        {
            "version": VERSION,
            "claim_id": "unsupported_real_navigation_sr_spl",
            "supported": False,
            "claim_boundary": "M88 does not execute Habitat trajectories, so it cannot claim real navigation SR/SPL.",
        },
        {
            "version": VERSION,
            "claim_id": "unsupported_human_intent_main_claim",
            "supported": False,
            "claim_boundary": "M88 detector policies are task-agnostic; structured task context is not tested as a main effect in this unit.",
        },
    ]
...
def build_route_decision_rows(ready: bool, uses_eval_policy: bool) -> list[dict[str, Any]]:
    if ready and not uses_eval_policy:
        return [
            {
                "version": VERSION,
                "decision": "proceed_after_source_gap_visit_order_path_smoke",
                "selected_next_unit": "E008-M89 source-gap leakage-safe detector candidate goal-evaluation smoke",
                "reason": "M88 materializes source-gap detector visit-order/path rows without eval-goal leakage; next step is eval-only goal-hit scoring for source-gap recovery.",
    ...
    return [
        {
            "version": VERSION,
            "decision": "repair_m88_source_gap_visit_order_path_smoke",
            "selected_next_unit": "repair E008-M88 source-gap visit-order/path smoke",
...
def build_report(
    ...
    source_gap_case_policy_rows: list[dict[str, Any]],
    ...
    case_lines = []
    for row in source_gap_case_policy_rows:
        case_lines.append(
            "| {scan_id} | {object_category} | {policy_id} | {ranked_candidate_rows} | {path_ready_ranked_rows} | "
            "{first_path_ready_rank} | {first_path_ready_cost_m} | {top5_path_ready_rows} |".format(
                **{key: format_value(row.get(key)) for key in row}
            )
        )
    failure_line = ", ".join(f"`{key}` {value}" for key, value in sorted(failure_counts.items())) or "none"
    return f"""# E008-M88 Source-Gap Detector Candidate Visit-Order Path Smoke
...
- Source-ready source-gap cases: {coverage['source_ready_source_gap_case_rows']} / {coverage['source_gap_case_rows']}.
...
- Source-gap case policy metric rows: {coverage['source_gap_case_policy_metric_rows']}.
...
## Source-Gap Case Policy Rows

| scan_id | category | policy_id | ranked | path-ready | first-ready rank | first-ready cost m | top5 path-ready |
| --- | --- | --- | ---: | ---: | ---: | ---: | ---: |
{chr(10).join(case_lines)}

## Claim Boundary

- M88 is a source-gap visit-order/path-cost smoke, not an executed navigation benchmark.
- M88 does not claim source-gap recovery because eval-only goal/viewpoint matching is not run here.
- M88 does not claim real navigation `SR` / `SPL`.
- M88 does not use `ObjectNav` goal/viewpoint coordinates as policy input.
...
def main() -> None:
    ...
    m87_coverage = read_json(M87_ARTIFACT_DIR / "coverage.json")
    candidate_rows = read_jsonl(M87_ARTIFACT_DIR / "candidate_navmesh_validation_rows.jsonl")
    source_gap_case_rows = read_jsonl(M87_ARTIFACT_DIR / "source_gap_case_source_ready_rows.jsonl")
    scan_source_rows = read_jsonl(M87_ARTIFACT_DIR / "scan_source_boundary_rows.jsonl")
    if not candidate_rows:
        raise SystemExit("missing E008-M87 candidate_navmesh_validation_rows.jsonl")
    if not source_gap_case_rows:
        raise SystemExit("missing E008-M87 source_gap_case_source_ready_rows.jsonl")

    source_gap_scan_ids = {str(row.get("scan_id")) for row in source_gap_case_rows}
    source_gap_candidate_rows = [row for row in candidate_rows if str(row.get("scan_id")) in source_gap_scan_ids]
    query_compatible_rows = [
        row
        for row in source_gap_candidate_rows
    ...
    source_gap_case_policy_rows = build_source_gap_case_policy_rows(scan_metric_rows, source_gap_case_rows)
    ...
    source_gap_cases_ready = all(row.get("source_ready_after_m86_m87") for row in source_gap_case_rows)
    uses_eval_policy = any(row.get("uses_objectnav_eval_goal_or_viewpoint_for_policy") for row in aggregate_rows)
    route_decision_rows = build_route_decision_rows(ready and source_gap_cases_ready, uses_eval_policy)
    claim_boundary_rows = build_claim_boundary_rows()

    coverage = {
        "version": VERSION,
        "status": "e008_m88_source_gap_detector_candidate_visit_order_path_smoke_ready"
        if ready and source_gap_cases_ready and not uses_eval_policy
        else "e008_m88_source_gap_detector_candidate_visit_order_path_smoke_blocked",
        "generated_at": datetime.now().isoformat(timespec="seconds"),
        "m87_status": m87_coverage.get("status"),
        "artifact_output_root": str(ARTIFACT_DIR),
        "derived_output_root": str(DATA_OUT_DIR),
        "input_candidate_rows": len(source_gap_candidate_rows),
        "query_compatible_candidate_rows": len(query_compatible_rows),
        "evaluated_scan_rows": len(rows_by_scan),
        "source_gap_case_rows": len(source_gap_case_rows),
        "source_ready_source_gap_case_rows": sum(1 for row in source_gap_case_rows if row.get("source_ready_after_m86_m87")),
        ...
        "source_gap_case_policy_metric_rows": len(source_gap_case_policy_rows),
        ...
        "candidate_visit_order_path_smoke_ready": ready and source_gap_cases_ready and not uses_eval_policy,
        "real_navigation_sr_spl_ready": False,
        "final_real_rgbd_open_vocab_robustness_ready": False,
    ...
    write_jsonl(ARTIFACT_DIR / "source_gap_case_policy_metric_rows.jsonl", source_gap_case_policy_rows)
    ...
    (ARTIFACT_DIR / "report.md").write_text(
        build_report(coverage, aggregate_rows, source_gap_case_policy_rows, failure_counts), encoding="utf-8"
    )
    ...
    write_jsonl(DATA_OUT_DIR / "source_gap_case_policy_metric_rows.jsonl", source_gap_case_policy_rows)
