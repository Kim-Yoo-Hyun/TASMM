#!/usr/bin/env python3
"""Build the E008-M139 confidence-preserving repair trajectory contract and Docker preflight."""
...
VERSION = "e008_m139_target_free_confidence_preserving_repair_trajectory_contract_v0"
READY_STATUS = "e008_m139_target_free_confidence_preserving_repair_trajectory_contract_ready_runner_next"
READY_RUNNER_MISSING_STATUS = "e008_m139_target_free_confidence_preserving_repair_trajectory_contract_ready_runner_missing"
BLOCKED_STATUS = "e008_m139_target_free_confidence_preserving_repair_trajectory_contract_blocked"

M138_DIR = EXP_ROOT / "artifacts" / "E008-M138_target_free_confidence_preserving_repair_materialization_smoke_v0"
ARTIFACT_DIR = EXP_ROOT / "artifacts" / "E008-M139_target_free_confidence_preserving_repair_trajectory_contract_v0"
DATA_OUT_DIR = (
    ROOT
    / "local_dataset"
...
M140_RUNNER = EXP_ROOT / "tools" / "run_m140_target_free_confidence_preserving_repair_trajectory_execution_smoke.py"
M140_ARTIFACT_DIR = (
    EXP_ROOT / "artifacts" / "E008-M140_target_free_confidence_preserving_repair_trajectory_execution_smoke_v0"
)
M140_DATA_OUT_DIR = (
    ROOT
    / "local_dataset"
    / "HM3D_navigation_bridge"
    / "E008-M140_target_free_confidence_preserving_repair_trajectory_execution_smoke_v0"
)
NEXT_UNIT = "E008-M140 target-free confidence-preserving repair trajectory execution smoke"
...
def build_execution_contract_rows(
    ...
    audit_index = {str(row.get("policy_id")): row for row in order_audit_rows}
        ...
        rows.append(
            {
                ...
                "first_confidence": summary.get("first_confidence"),
                "first_current_pose_to_candidate_geodesic_m": summary.get(
                    "first_current_pose_to_candidate_geodesic_m"
                ),
                "detector_order_identical": audit.get("detector_order_identical"),
                ...
                "runner_script": str(M140_RUNNER.relative_to(ROOT)),
                ...
                "execute_in_m140": True,
                "uses_objectnav_eval_goal_or_viewpoint_for_policy": False,
                "uses_objectnav_eval_goal_or_viewpoint_for_metric": True,
                "claim_boundary": "M139 fixes the execution contract only; M140 is required for executed SR/SPL.",
            }
        )
    return rows
...
def build_docker_preflight_rows(
    ...
    m140_compile: dict[str, Any] | None,
) -> list[dict[str, Any]]:
    ...
    return [
        ...
        {
            "version": VERSION,
            "check_id": "m140_confidence_preserving_runner_available",
            "status": "pass" if M140_RUNNER.exists() and m140_compile and m140_compile.get("ok") else "fail",
            "evidence": f"runner={M140_RUNNER.relative_to(ROOT)}; exists={M140_RUNNER.exists()}; py_compile={bool(m140_compile and m140_compile.get('ok'))}.",
        },
    ]
...
def build_m140_command_rows() -> list[dict[str, Any]]:
    command = (
        ...
        "experiments/E008_real_navigation_benchmark/tools/run_m140_target_free_confidence_preserving_repair_trajectory_execution_smoke.py "
        "--m139-contract experiments/E008_real_navigation_benchmark/artifacts/E008-M139_target_free_confidence_preserving_repair_trajectory_contract_v0 "
        "--out-root experiments/E008_real_navigation_benchmark/artifacts/E008-M140_target_free_confidence_preserving_repair_trajectory_execution_smoke_v0 "
        "--derived-out-root local_dataset/HM3D_navigation_bridge/E008-M140_target_free_confidence_preserving_repair_trajectory_execution_smoke_v0\""
    )
    return [
        {
            "version": VERSION,
            ...
            "runner_implemented": M140_RUNNER.exists(),
            "output_path": str(M140_ARTIFACT_DIR.relative_to(ROOT)),
            "derived_output_path": str(M140_DATA_OUT_DIR.relative_to(ROOT)),
            "command": command,
            "launch_now": False,
            "expected_files": [
                "coverage.json",
            ...
            "verification_command": (
                ...
                "import json\n"
                "from pathlib import Path\n"
                "p=Path('experiments/E008_real_navigation_benchmark/artifacts/E008-M140_target_free_confidence_preserving_repair_trajectory_execution_smoke_v0/coverage.json')\n"
                "c=json.loads(p.read_text())\n"
                "assert c['status']=='e008_m140_target_free_confidence_preserving_repair_trajectory_execution_smoke_ready'\n"
                "assert c['scan_task_policy_rows'] == 6\n"
                "print('m140 ready')\n"
                "PY"
            ),
        }
...
def build_readiness_gate_rows(
    ...
    selected_audit = next((row for row in order_audit_rows if row.get("policy_id") == METHOD_POLICY), {})
    gates = [
        (
            "m138_materialization_ready",
            m138_cov.get("status") == "e008_m138_target_free_confidence_preserving_repair_materialization_smoke_ready",
            f"M138 status={m138_cov.get('status')}.",
            True,
        ),
        (
        ...
        (
            ...
            len(plan_rows) == EXPECTED_PLAN_ROWS and len(plan_counts) == EXPECTED_POLICY_COUNT,
            f"plan rows={len(plan_rows)}; policies={dict(sorted(plan_counts.items()))}.",
            True,
        ),
        (
        ...
        (
            ...
            bool(selected_audit) and selected_audit.get("audit_pass") and selected_audit.get("confidence_band_violation_count") == 0,
        ...
        (
            "high_hard_veto_count_recorded",
            True,
            f"selected hard-veto count={selected_audit.get('hard_feasibility_veto_count')}; this is a warning for M140 interpretation, not a M139 blocker.",
            False,
        ),
        (
            "execute_trajectories_now",
            False,
            "M139 is a contract/preflight unit; M140 should execute.",
            False,
        ),
    ]
    return [
        {
            ...
            "passed": passed,
            "blocks_m140": blocks and not passed,
            "evidence": evidence,
        }
...
def build_claim_boundary_rows() -> list[dict[str, Any]]:
    return [
        {
            "version": VERSION,
            "claim_id": "confidence_preserving_repair_execution_contract_ready",
            "status": "supported_contract_only",
            "safe_claim": "M139 provides a Docker-preflighted, runner-compatible one-case confidence-preserving repair execution contract.",
        },
        {
            "version": VERSION,
            "claim_id": "confidence_preserving_repair_execution",
            "status": "not_executed",
            "safe_claim": "M139 does not execute Habitat trajectories; M140 is required for SR/SPL.",
        },
        {
            "version": VERSION,
            "claim_id": "final_real_navigation_sr_spl",
            "status": "blocked",
            "safe_claim": "Final real navigation claim needs M140 execution, result interpretation, larger scale, and external navigation/search baselines.",
        },
        {
            "version": VERSION,
            "claim_id": "human_intent_main_claim",
            "status": "blocked",
            "safe_claim": "M139 target-free repair contract does not use human intent; E006-M08 remains the active human-intent boundary.",
        },
    ]
...
def build_route_decision_rows(contract_ready: bool, runner_ready: bool) -> list[dict[str, Any]]:
    if contract_ready and runner_ready:
        ...
        decision = "run_m140_target_free_confidence_preserving_repair_trajectory_smoke"
    elif contract_ready:
        selected_next = "E008-M140 target-free confidence-preserving repair trajectory execution runner scaffold"
        decision = "scaffold_m140_runner_before_docker_execution"
    else:
        selected_next = "repair E008-M139 target-free confidence-preserving repair trajectory contract"
        decision = "repair_m139_contract_or_preflight"
    return [
        {
            "version": VERSION,
            "row_type": "route_decision",
            "decision_id": "m139_selected_next",
            "decision": decision,
            "selected_next_unit": selected_next,
            "launch_long_job_now": False,
            "reason": "M139 fixes the confidence-preserving repair execution input contract and Docker/data preflight; M140 should execute the bounded one-case trajectory smoke."
            if contract_ready
            else "One or more M139 contract/preflight gates failed.",
        }
    ]
...
def build_report(
    ...
    return "\n".join(
        [
            "# E008-M139 Target-Free Confidence-Preserving Repair Trajectory Contract",
            "",
            ...
            "## Gates",
            "",
            markdown_table(gate_rows, ["gate_id", "status", "blocks_m140", "evidence"]),
            "",
            "## Docker Preflight",
            "",
            ...
            "",
            "- M139 supports only the confidence-preserving repair execution contract and Docker/data preflight.",
            "- M139 intentionally does not claim final real navigation `SR` / `SPL`, deployable search policy, or final real RGB-D/open-vocabulary robustness.",
...
def copy_input_files(output_dir: Path) -> None:
    ...
    for filename in CORE_INPUT_FILES + M138_AUX_FILES:
        src = M138_DIR / filename
        if src.exists():
...
def main() -> None:
    ...
    m138_cov = read_json(M138_DIR / "coverage.json")
    candidate_rows = read_jsonl(M138_DIR / "dynamic_stale_overlay_trajectory_candidate_rows.jsonl")
    plan_rows = read_jsonl(M138_DIR / "trajectory_execution_plan_rows.jsonl")
    goal_rows = read_jsonl(M138_DIR / "episode_goal_eval_rows.jsonl")
    oracle_rows = read_jsonl(M138_DIR / "oracle_path_rows.jsonl")
    matrix_rows = read_jsonl(M138_DIR / "trajectory_cost_matrix_rows.jsonl")
    policy_summary_rows = read_jsonl(M138_DIR / "policy_summary_rows.jsonl")
    order_audit_rows = read_jsonl(M138_DIR / "policy_order_audit_rows.jsonl")

    if not m138_cov:
        raise SystemExit("missing M138 coverage.json")
    if not all([candidate_rows, plan_rows, goal_rows, oracle_rows, matrix_rows, policy_summary_rows, order_audit_rows]):
        raise SystemExit("missing one or more required M138 rows for M139")

    contract_rows = build_execution_contract_rows(plan_rows, candidate_rows, policy_summary_rows, order_audit_rows)
    ...
    m140_compile = command_status(["python", "-m", "py_compile", str(M140_RUNNER)], timeout_s=30) if M140_RUNNER.exists() else None
    ...
    contract_ready = not any(row.get("blocks_m140") for row in gate_rows)
    runner_ready = M140_RUNNER.exists() and bool(m140_compile and m140_compile.get("ok"))
    route_rows = build_route_decision_rows(contract_ready, runner_ready)
    command_rows = build_m140_command_rows()
    ...
    coverage = {
        ...
        "m138_status": m138_cov.get("status"),
    ...
    for output_dir in (ARTIFACT_DIR, DATA_OUT_DIR):
        ...
        write_jsonl(output_dir / "m140_command_rows.jsonl", command_rows)
