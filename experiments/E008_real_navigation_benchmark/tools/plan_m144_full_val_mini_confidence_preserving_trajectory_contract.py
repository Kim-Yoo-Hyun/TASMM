#!/usr/bin/env python3
"""Build E008-M144 full-val-mini confidence-preserving trajectory execution contract."""
...
VERSION = "e008_m144_full_val_mini_confidence_preserving_trajectory_contract_v0"
READY_STATUS = "e008_m144_full_val_mini_confidence_preserving_trajectory_contract_ready_runner_next"
READY_RUNNER_MISSING_STATUS = "e008_m144_full_val_mini_confidence_preserving_trajectory_contract_ready_runner_missing"
BLOCKED_STATUS = "e008_m144_full_val_mini_confidence_preserving_trajectory_contract_blocked"

M143_DIR = EXP_ROOT / "artifacts" / "E008-M143_full_val_mini_confidence_preserving_trajectory_cost_materialization_v0"
ARTIFACT_DIR = EXP_ROOT / "artifacts" / "E008-M144_full_val_mini_confidence_preserving_trajectory_contract_v0"
DATA_OUT_DIR = (
    ROOT
    / "local_dataset"
    / "HM3D_navigation_bridge"
    / "E008-M144_full_val_mini_confidence_preserving_trajectory_contract_v0"
)
...
M145_RUNNER = EXP_ROOT / "tools" / "run_m145_full_val_mini_confidence_preserving_trajectory_execution.py"
M145_ARTIFACT_DIR = (
    EXP_ROOT / "artifacts" / "E008-M145_full_val_mini_confidence_preserving_trajectory_execution_v0"
)
M145_DATA_OUT_DIR = (
    ROOT
    / "local_dataset"
    / "HM3D_navigation_bridge"
    / "E008-M145_full_val_mini_confidence_preserving_trajectory_execution_v0"
)
NEXT_UNIT = "E008-M145 full-val-mini confidence-preserving trajectory execution"
...
M143_AUX_FILES = [
    ...
    "policy_order_audit_summary_rows.jsonl",
...
def final_plan_rows(plan_rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    by_policy: dict[str, list[dict[str, Any]]] = defaultdict(list)
    for row in plan_rows:
        by_policy[str(row.get("policy_id"))].append(row)
    return [max(rows, key=lambda row: finite_float(row.get("planned_cumulative_path_cost_m")) or -math.inf) for rows in by_policy.values()]
...
def build_execution_contract_rows(
    ...
    audit_index = {str(row.get("policy_id")): row for row in audit_summary_rows}
    final_plan_index = {str(row.get("policy_id")): row for row in final_plan_rows(plan_rows)}
    rows: list[dict[str, Any]] = []
    for policy_id in sorted(plan_counts):
        summary = summary_index.get(policy_id, {})
        audit = audit_index.get(policy_id, {})
        final_plan = final_plan_index.get(policy_id, {})
        rows.append(
            {
                "version": VERSION,
                "row_type": "trajectory_execution_contract",
                "policy_id": policy_id,
                "policy_role": summary.get("policy_role") or final_plan.get("policy_role"),
                ...
                "max_plan_candidate_budget": final_plan.get("candidate_budget"),
                "max_plan_scene_key": final_plan.get("scene_key"),
                "max_plan_adapter_episode_id": final_plan.get("adapter_episode_id"),
                "detector_order_identical_episode_rows": audit.get("detector_order_identical_rows"),
                "hard_feasibility_veto_count": audit.get("hard_feasibility_veto_count"),
                "confidence_band_violation_count": audit.get("confidence_band_violation_count"),
                "runner_script": str(M145_RUNNER.relative_to(ROOT)),
                ...
                "execute_in_m145": True,
                "uses_objectnav_eval_goal_or_viewpoint_for_policy": False,
                "uses_objectnav_eval_goal_or_viewpoint_for_metric": True,
                "claim_boundary": "M144 fixes the full-val-mini execution contract only; M145 is required for executed SR/SPL.",
            }
        )
    return rows
...
def build_docker_preflight_rows(
    ...
    m145_compile: dict[str, Any] | None,
) -> list[dict[str, Any]]:
    ...
    return [
        ...
        {
            "version": VERSION,
            "check_id": "m145_full_val_mini_runner_available",
            "status": "pass" if M145_RUNNER.exists() and m145_compile and m145_compile.get("ok") else "fail",
            "evidence": f"runner={M145_RUNNER.relative_to(ROOT)}; exists={M145_RUNNER.exists()}; py_compile={bool(m145_compile and m145_compile.get('ok'))}.",
        },
    ]


def build_m145_command_rows() -> list[dict[str, Any]]:
    command = (
        ...
        "experiments/E008_real_navigation_benchmark/tools/run_m145_full_val_mini_confidence_preserving_trajectory_execution.py "
        "--m144-contract experiments/E008_real_navigation_benchmark/artifacts/E008-M144_full_val_mini_confidence_preserving_trajectory_contract_v0 "
        "--out-root experiments/E008_real_navigation_benchmark/artifacts/E008-M145_full_val_mini_confidence_preserving_trajectory_execution_v0 "
    ...
    return [
        {
            "version": VERSION,
            "command_id": "e008_m145_full_val_mini_confidence_preserving_trajectory_execution",
            ...
            "runner_path": str(M145_RUNNER.relative_to(ROOT)),
            "runner_implemented": M145_RUNNER.exists(),
            "output_path": str(M145_ARTIFACT_DIR.relative_to(ROOT)),
            "derived_output_path": str(M145_DATA_OUT_DIR.relative_to(ROOT)),
            "command": command,
            "launch_now": False,
            "expected_files": [
                "coverage.json",
            ...
            "verification_command": (
                ...
                "import json\n"
                "from pathlib import Path\n"
                "p=Path('experiments/E008_real_navigation_benchmark/artifacts/E008-M145_full_val_mini_confidence_preserving_trajectory_execution_v0/coverage.json')\n"
                "c=json.loads(p.read_text())\n"
                "assert c['status']=='e008_m145_full_val_mini_confidence_preserving_trajectory_execution_ready'\n"
...
def build_readiness_gate_rows(
    ...
    candidate_counts = Counter(str(row.get("policy_id")) for row in candidate_rows)
    plan_counts = Counter(str(row.get("policy_id")) for row in plan_rows)
    selected_audit = next((row for row in audit_summary_rows if row.get("policy_id") == METHOD_POLICY), {})
    ...
    gates = [
        (
            ...
            f"M143 status={m143_cov.get('status')}.",
            True,
        ),
        (
        ...
        (
            ...
            bool(selected_audit)
            and selected_audit.get("audit_pass")
            and selected_audit.get("confidence_band_violation_count") == 0,
            f"selected_audit={selected_audit}.",
            True,
        ),
        (
        ...
        (
            ...
            False,
            "M144 is a contract/preflight unit; M145 should execute.",
            False,
        ),
    ]
    return [
        {
            ...
            "passed": passed,
            "blocks_m145": blocks and not passed,
            "evidence": evidence,
        }
...
def build_claim_boundary_rows() -> list[dict[str, Any]]:
    return [
        {
            "version": VERSION,
            "claim_id": "full_val_mini_confidence_preserving_execution_contract_ready",
            "status": "supported_contract_only",
            "safe_claim": "M144 provides a Docker-preflighted, runner-compatible full-val-mini confidence-preserving trajectory execution contract.",
        },
        {
            "version": VERSION,
            "claim_id": "full_val_mini_confidence_preserving_execution",
            "status": "not_executed",
            "safe_claim": "M144 does not execute Habitat trajectories; M145 is required for SR/SPL.",
        },
        {
            "version": VERSION,
            "claim_id": "final_real_navigation_sr_spl",
            "status": "blocked",
            "safe_claim": "Final real navigation claim needs M145 execution, M146 interpretation, heldout transfer, and external navigation/search baselines.",
        },
        {
            "version": VERSION,
            "claim_id": "human_intent_main_claim",
            "status": "blocked",
            "safe_claim": "M144 target-free execution contract does not use human intent; E006-M08 remains the active human-intent boundary.",
        },
    ]
...
def build_route_decision_rows(contract_ready: bool, runner_ready: bool) -> list[dict[str, Any]]:
    if contract_ready and runner_ready:
        ...
        decision = "run_m145_full_val_mini_confidence_preserving_trajectory_execution"
    elif contract_ready:
        selected_next = "E008-M145 full-val-mini confidence-preserving trajectory execution runner scaffold"
        decision = "scaffold_m145_runner_before_docker_execution"
    else:
        selected_next = "repair E008-M144 full-val-mini confidence-preserving trajectory contract"
        decision = "repair_m144_contract_or_preflight"
    return [
        {
            "version": VERSION,
            "row_type": "route_decision",
            "decision_id": "m144_selected_next",
            "decision": decision,
            "selected_next_unit": selected_next,
            "launch_long_job_now": False,
            "reason": "M144 fixes the full-val-mini confidence-preserving execution input contract and Docker/data preflight; M145 should execute trajectories."
            if contract_ready
            else "One or more M144 contract/preflight gates failed.",
        }
    ]
...
def build_report(
    ...
    return "\n".join(
        [
            "# E008-M144 Full-Val-Mini Confidence-Preserving Trajectory Contract",
            "",
            ...
            f"- Confidence-band violations: {coverage['selected_policy_confidence_band_violations']}.",
            ...
            "## Gates",
            "",
            markdown_table(gate_rows, ["gate_id", "status", "blocks_m145", "evidence"]),
            "",
            "## Docker Preflight",
            "",
            ...
            "",
            "- M144 supports only the full-val-mini confidence-preserving execution contract and Docker/data preflight.",
            "- M144 intentionally does not claim final real navigation `SR` / `SPL`, deployable search policy, or final real RGB-D/open-vocabulary robustness.",
            "- M145 must test whether the confidence-preserving policy still protects detector-confidence `SR` / `SPL` after scaling from one case to 30 episodes.",
            "",
        ]
    )
...
def copy_input_files(output_dir: Path) -> None:
    ...
    for filename in CORE_INPUT_FILES + M143_AUX_FILES:
        src = M143_DIR / filename
        if src.exists():
...
def main() -> None:
    ...
    base_candidate_rows = read_jsonl(M143_DIR / "base_candidate_rows.jsonl")
    candidate_rows = read_jsonl(M143_DIR / "dynamic_stale_overlay_trajectory_candidate_rows.jsonl")
    plan_rows = read_jsonl(M143_DIR / "trajectory_execution_plan_rows.jsonl")
    goal_rows = read_jsonl(M143_DIR / "episode_goal_eval_rows.jsonl")
    oracle_rows = read_jsonl(M143_DIR / "oracle_path_rows.jsonl")
    matrix_rows = read_jsonl(M143_DIR / "trajectory_cost_matrix_rows.jsonl")
    policy_summary_rows = read_jsonl(M143_DIR / "policy_summary_rows.jsonl")
    audit_summary_rows = read_jsonl(M143_DIR / "policy_order_audit_summary_rows.jsonl")

    if not m143_cov:
        raise SystemExit("missing M143 coverage.json")
    if not all(
        [
            base_candidate_rows,
            candidate_rows,
        ...
        raise SystemExit("missing one or more required M143 rows for M144")

    contract_rows = build_execution_contract_rows(plan_rows, candidate_rows, policy_summary_rows, audit_summary_rows)
    ...
    m145_compile = command_status(["python", "-m", "py_compile", str(M145_RUNNER)], timeout_s=30) if M145_RUNNER.exists() else None
    ...
    contract_ready = not any(row.get("blocks_m145") for row in gate_rows)
    runner_ready = M145_RUNNER.exists() and bool(m145_compile and m145_compile.get("ok"))
    route_rows = build_route_decision_rows(contract_ready, runner_ready)
    command_rows = build_m145_command_rows()
    ...
    for output_dir in (ARTIFACT_DIR, DATA_OUT_DIR):
        ...
        write_jsonl(output_dir / "m145_command_rows.jsonl", command_rows)
