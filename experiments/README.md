...
Main experiment implementation stage has started. E001-M01 through E001-M05, E002-M01 through E002-M09, E003-M00 through E003-M75, E004-M01 through E004-M05, E005-M01 through E005-M101, E007-M01 through E007-M07, and E008-M01 through E008-M45 are complete/verified with constraints through denominator-aligned `Open3DSG`, full-denominator real proposal diagnostics, `ConceptGraphs` reliability boundary, row-group/heavier-route decision, `ConceptGraphs`-assisted H001 fallback policy smoke, map-assisted fallback claim-boundary decision, navigation/path-cost bridge packaging, real navigation source preflight, episode/source adapter smoke, H001 candidate-to-navigation adapter contract, ObjectNav oracle path/metric smoke, rendered RGB-D detector-source planning, expanded detector route, trajectory-execution contract/Docker preflight, detector-policy trajectory execution smoke, proxy/trajectory consistency, H001 candidate-source contract, H001 candidate-source materialization, H001 visit-order/path smoke, H001 leakage-safe goal-evaluation smoke, H001 goal-evaluation comparison / trajectory decision, H001 current-observation fallback/source repair contract, H001 fallback replay smoke, H001 fallback trajectory contract/source-gap boundary, H001 fallback trajectory execution smoke, H001 trajectory result interpretation/baseline alignment, dynamic-stale navigation benchmark/source-intervention contract, dynamic-stale overlay materialization, dynamic-stale overlay trajectory contract/runner adaptation, dynamic-stale overlay trajectory execution smoke, M38 result interpretation/baseline alignment, M39 budget-matched repair/source-gap contract, M40 repaired row materialization, M41 repaired trajectory execution, M42 result interpretation/scale decision, M43 source-diverse policy redesign contract, M44 source-diverse row materialization, and M45 source-diverse trajectory execution contract/Docker preflight. E008-M45 fixes Docker-ready execution for `h001_task_conditioned_source_diverse_budget5_v1` and matched baselines as 108 execution plan rows / 468 candidate rows with Docker/data/runner/source-gap reporting checks passing. Main navigation table, final real navigation `SR` / `SPL`, `OldLocationDeadEndCostM` as a primary metric, final real RGB-D/open-vocabulary robustness, and human intent main claim remain blocked. Next unit is E008-M46 source-diverse redesign trajectory execution smoke.
...
| E008 | M01-M45 navigation source/adapter/oracle metric/staging, rendered detector-source plan, detector candidates, navmesh validation, visit-order path smoke, goal-evaluation smoke, non-oracle observation expansion, expanded detector route, trajectory-execution contract, detector-policy trajectory smoke, proxy/trajectory consistency, H001 source contract, H001 source materialization, H001 visit-order/path smoke, H001 goal-evaluation smoke, H001 trajectory gate decision, H001 fallback/source repair contract, fallback replay smoke, H001 fallback trajectory contract/source-gap boundary, H001 fallback trajectory execution smoke, H001 trajectory result interpretation/baseline alignment, dynamic-stale navigation contract, overlay materialization, runner adaptation, trajectory smoke, result interpretation, budget-matched repair/source-gap contract, repaired row materialization/execution, result interpretation/scale decision, source-diverse policy redesign contract, source-diverse row materialization, and trajectory contract/Docker preflight ready with final H001 navigation blocked | [E008_real_navigation_benchmark](E008_real_navigation_benchmark/README.md) | E008-M46 source-diverse redesign trajectory execution smoke |
...
- E005-M49 aggregates full 9-scan heldout `ConceptGraphs` metrics: strict bbox top5 114 / 195 and relaxed bbox 1m top3 144 / 195.
...
E005 shows that `DualMap` can execute on the staged `3RScan` adapter, but the current route does not produce object-map `*.pkl` outputs. `ConceptGraphs` is the active converted positive external mapping baseline route and has full 9-scan heldout query-level aggregation plus H001 replay on the same query contract. `Open3DSG` has a denominator-aligned bridge and corrected query metrics; M64 verifies that a bounded predicted-vocabulary adapter repairs much of the primary-label gap. E007-M07 packages the path-cost bridge as paper-facing occupancy-grid proxy evidence with reviewer sensitivity checks. E008-M45 confirms that source-diverse budget-5 rows can be promoted to a Docker-ready trajectory contract without policy leakage or source-gap reporting loss; the next evidence must come from Docker trajectory execution.

## 사용자 판단 필요

Proceed to E008-M46 source-diverse redesign trajectory execution smoke before broader navigation reruns.
