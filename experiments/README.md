...
| E008 | M01-M193 ready with constraints; M194 render ready, detector running | [E008_real_navigation_benchmark](E008_real_navigation_benchmark/README.md) | E008-M194 detector completion verification |
...
E005 shows that `DualMap` can execute on the staged `3RScan` adapter, but the current route does not produce object-map `*.pkl` outputs. `ConceptGraphs` is the active converted positive external mapping baseline route and has full 9-scan heldout query-level aggregation plus H001 replay on the same query contract. `Open3DSG` has a denominator-aligned bridge and corrected query metrics; M64 verifies that a bounded predicted-vocabulary adapter repairs much of the primary-label gap. E007-M07 packages the path-cost bridge as paper-facing occupancy-grid proxy evidence with reviewer sensitivity checks. E008-M54 shows the repaired `routine_fetch` policy is executable and paper-table-packaged as diagnostic evidence, but task context and final navigation are not positive claims because H001 v2 ties task-agnostic source-diverse and loses `SPL` to detector/fixed baselines.

## 사용자 판단 필요

Proceed to E008-M55 source-gap candidate-generation repair feasibility decision before broader navigation reruns.
