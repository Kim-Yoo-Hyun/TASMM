#!/usr/bin/env python3
"""Verify the E005-M11 DualMap detector-enabled free-GPU retry background job."""
...
VERIFY_VERSION = "e005_m11_dualmap_detector_retry_verifier_v0"
...
def detect_failure_signals(log_tail: str, background_status: dict[str, Any]) -> list[str]:
    ...
    if "Detector][Init] Error loading YOLO model" in log_tail:
        signals.append("yolo_model_init_failed")
    if "Detector][Init] Error loading SAM model" in log_tail:
        signals.append("sam_model_init_failed")
    if "Detector][Init] Error loading FASTSAM model" in log_tail:
    ...
    if "No such file or directory" in log_tail:
        signals.append("missing_file_or_path")
    if "Permission denied" in log_tail:
        signals.append("permission_denied")
    if "Killed" in log_tail:
        signals.append("process_killed")
...
def build_report(coverage: dict[str, Any]) -> str:
    ...
    lines = [
        "# E005-M11 DualMap Detector-Enabled Retry Verification",
        "",
        "## Status",
        "",
        coverage["status"],
        ...
        f"- `detector_time.csv` count: {inv['detector_time_count']}.",
        ...
        "",
        "- E005-M11 verification only supports runtime-smoke readiness or failure diagnosis.",
        "- Runtime file presence alone does not support `DualMap` baseline performance.",
        "- If object `*.pkl` is produced, inspect its schema before building an E004/E005 adapter.",
        ...
        "- If runtime outputs are ready, the next unit is object schema inspection.",
        "- If runtime failed again, choose loader-only layout smoke or lower-memory detector retry before `ConceptGraphs` fallback.",
        "",
        "## User Decision Needed",
        "",
        "- None before the next bounded verification or repair route.",
        "",
    ]
    return "\n".join(lines)
...
def main() -> int:
    ...
    if running:
        status = "e005_m11_dualmap_detector_retry_running"
    elif inventory["pkl_count"] > 0 and inventory["layout_pcd_count"] > 0 and inventory["system_time_count"] > 0:
        status = "e005_m11_dualmap_detector_retry_outputs_ready"
    elif background_status.get("status") == "failed" or failure_signals:
        status = "e005_m11_dualmap_detector_retry_failed"
    elif background_status.get("status") == "completed":
        status = "e005_m11_dualmap_detector_retry_completed_missing_expected_outputs"
    else:
        status = "e005_m11_dualmap_detector_retry_needs_verification"
    ...
    return 0 if status in {"e005_m11_dualmap_detector_retry_running", "e005_m11_dualmap_detector_retry_outputs_ready"} else 1
