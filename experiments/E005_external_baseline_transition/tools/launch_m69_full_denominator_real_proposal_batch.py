#!/usr/bin/env python3
"""Launch an E005-M69 full-denominator real proposal detector batch."""
...
OUT_DIR = EXP_ROOT / "artifacts" / "E005-M69_full_denominator_real_proposal_detector_launch_v0"
VERSION = "e005_m69_full_denominator_real_proposal_detector_launch_v0"
...
def build_report(coverage: dict[str, Any]) -> str:
    return "\n".join(
        [
            "# E005-M69 Full-Denominator Real Proposal Detector Launch",
            "",
            "## Facts",
            "",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--m68-dir", default=M68_DIR, type=Path)
    ...
    parser.add_argument("--sudo-password-env", default="E005_M69_SUDO_PASSWORD")
...
def main() -> int:
    args = parse_args()
    args.m68_dir = args.m68_dir.resolve()
    args.out_dir = args.out_dir.resolve() / args.batch_id
    args.out_dir.mkdir(parents=True, exist_ok=True)

    command_plan_path = args.m68_dir / "batches" / args.batch_id / "detector_run_command_plan.json"
    command_plan = read_json(command_plan_path)
    m68_coverage = read_json(args.m68_dir / "coverage.json")
    if m68_coverage.get("status") != "e005_m68_full_denominator_real_proposal_bridge_plan_ready":
        raise RuntimeError(f"M68 is not ready: {m68_coverage.get('status')}")
    if not command_plan:
        raise RuntimeError(f"missing command plan: {command_plan_path}")

    exact_command = [str(part) for part in command_plan["exact_command"]]
    tmux_session = str(command_plan["tmux_session"])
    ...
    log_path = ROOT / "logs" / f"{timestamp}_e005_m69_real_proposal_{args.batch_id}.log"
    log_path.parent.mkdir(parents=True, exist_ok=True)
    run_script = args.out_dir / f"run_m69_{args.batch_id}.sh"
    ...
    if not tmux_path:
        ...
        fifo_path = Path("/tmp") / f"e005_m69_sudo_{args.batch_id}_{os.getpid()}_{timestamp}.fifo"
        if needs_sudo_password:
        ...
        if proc.returncode == 0 and password_delivery["ok"] and after_running:
            ...
            status = "e005_m69_real_proposal_detector_job_exited_needs_verification"
    ...
    coverage = {
        ...
        "e005_m69_version": VERSION,
        "exact_command": exact_command,
        "expected_files": command_plan.get("expected_files"),
        "input_dir": command_plan.get("input_dir"),
        ...
        "next_recommended_unit": "E005-M70 full-denominator real proposal detector completion verification",
        ...
        "status": status,
        "sudo_password_recorded": sudo_password is not None,
        "tmux_session": tmux_session,
        "verification_command": command_plan.get("verification_command"),
