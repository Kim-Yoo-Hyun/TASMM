#!/usr/bin/env python3
"""Launch the E003-M74 expanded direct bridge detector run in tmux."""
...
DEFAULT_OUT_DIR = EXPERIMENT_ROOT / "artifacts" / "E003-M74_direct_bridge_denominator_detector_launch_v0"
M74_VERSION = "e003_m74_direct_bridge_denominator_detector_launch_v0"
TMUX_SESSION = "e003_m74_direct_denominator"
...
def build_report(coverage: dict[str, Any]) -> str:
    return "\n".join(
        [
            ...
            f"- detector-ready query rows: {coverage['detector_ready_query_rows']}.",
            ...
            "- Verify completion with expected files, schema validation, matching coverage, and targeted log tail.",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--sudo-password-env", default="E003_M74_SUDO_PASSWORD")
...
def main() -> int:
    args = parse_args()
    args.m73_dir = args.m73_dir.resolve()
    args.out_dir = args.out_dir.resolve()
    args.out_dir.mkdir(parents=True, exist_ok=True)

    command_plan = load_json(args.m73_dir / "detector_run_command_plan.json")
    ...
    log_path = REPO_ROOT / "logs" / f"{timestamp}_e003_m74_direct_bridge_denominator_detector_run.log"
    run_script = args.out_dir / "run_m74_detector.sh"
    ...
    if not tmux_path:
        status = "expanded_direct_bridge_detector_launch_failed"
        background_status = "failed"
        launch_stderr = "tmux_not_found"
    elif before_running:
        status = "expanded_direct_bridge_detector_job_already_running"
        ...
        fifo_path = Path("/tmp") / f"e003_m74_sudo_{os.getpid()}_{timestamp}.fifo"
        if needs_sudo_password:
        ...
        if proc.returncode == 0 and password_delivery["ok"] and after_running:
            ...
            status = "expanded_direct_bridge_detector_job_exited_needs_verification"
    ...
    coverage = {
        ...
        "command_plan": str(args.m73_dir / "detector_run_command_plan.json"),
        "detector_ready_query_rows": m73_coverage.get("detector_ready_query_rows"),
        "e003_m74_version": M74_VERSION,
        ...
        "next_recommended_unit": "E003-M74 completion verification after tmux exits",
        "output_dir": command_plan.get("output_dir"),
        "password_delivery": password_delivery,
        "prompt_label_count": m73_coverage.get("prompt_label_count"),
        "prompt_labels": m73_coverage.get("prompt_labels"),
        "run_script": str(run_script),
        "status": status,
        "sudo_password_recorded": sudo_password is not None,
        "target_scan_count": len(m73_coverage.get("selected_scan_ids", [])),
        "target_scan_ids": m73_coverage.get("selected_scan_ids", []),
