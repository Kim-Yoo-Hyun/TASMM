...
- Current claim state: E005-M56-M101 use `/home/yoohyun/research/local_dataset/Open3DSG_staged` as a read-only source and store derived outputs under `/home/yoohyun/research2/local_dataset/Open3DSG_bridge/`. E005-M100 selects `h001_then_conceptgraphs_top5_on_observed_miss_v0`: H001 success 157 / 195 -> 181 / 195, `AttemptSPL` proxy 0.773932 -> 0.798675, mean `ExpectedSearchCost` 1.758974 -> 2.435897. E006-M06 materializes 10,400 frozen baseline policy rows with leakage fail rows 0, but no utility or transfer claim. E007-M07 packages the E007 bridge table as paper-facing occupancy-grid path-cost proxy evidence with 6 table rows, 3 allowed claim rows, and 3 blocked claim rows. E008-M01-M122 now cover local read-only `HM3D ObjectNav` + `Habitat` source preflight through target-free render/detector launcher contract. M122 writes M123/M124 launcher input materialization rows and long-job command rows with ObjectNav target/viewpoint source-placement leakage false, readiness fail rows 0, and warning rows 1. E008-M123 was relaunched under GPU-available conditions and generated 320 / 320 color/depth/pose files, but verification failed with ready frames 295 / 320 because 25 depth frames failed positive-depth validation. Source-gap recovery, final real RGB-D/open-vocabulary robustness, and final real navigation remain blocked until downstream render/detector, trajectory, and baseline evaluation.
...
| E008-M123 target-free render frame staging launch / verification | `python experiments/E008_real_navigation_benchmark/tools/launch_m123_target_free_render_frame_staging.py`; `python experiments/E008_real_navigation_benchmark/tools/verify_m123_target_free_render_frame_staging.py --require-ready` | Inspect `experiments/E008_real_navigation_benchmark/artifacts/E008-M123_target_free_source_coverage_render_frame_staging_launch_v0/launch_coverage.json`, `coverage.json`, `verification_issue_rows.jsonl`, `job_status_rows.jsonl`, and `report.md`; latest verification failed with ready frames 295 / 320 because 25 depth frames failed positive-depth validation |
...
2026-06-05에 지정된 `DualMap` local runtime/cache paths는 삭제 완료했다. 아래 경로는 삭제/보존 상태 점검용 목록이다.
