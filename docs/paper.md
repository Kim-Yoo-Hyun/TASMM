...
Latest method-pivot update: E008-M166-M191 are complete. E008-M166-M176 freeze the local-rerank failure boundary and define source coverage as a semantic-map trigger for candidate-source expansion / re-observation before detector-confidence ranking. E008-M177-M184 validate that fixed-budget source-pool acquisition can surface recoverable candidates and execute bounded Docker trajectories. E008-M185-M190 show that ranking must remain protected by detector confidence because path/transition-cost orders do not improve `SPL`; E008-M191 locks the next paper gate around source-pool acquisition, protected detector-confidence default, and a no-source-pool ablation.
