...
| The system improves real navigation `SR` / `SPL` | simulator/navmesh/trajectory execution and navigation baselines | unsupported | E008-M12 provides leakage-safe `GoalEvalProxy` over 6 `HM3D ObjectNav` rows, but no trajectory execution or real `SR` / `SPL` yet |
