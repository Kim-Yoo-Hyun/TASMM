...
Latest navigation ledger update: E008-M136 is complete with status `e008_m136_target_free_trajectory_aware_repair_result_interpretation_scale_decision_ready`: M135 executed 5 policy trajectories over the one target-free repair case. All policies reach `SR` 1.0, but the selected repair policy `trajectory_greedy_confidence_path_repair_v0` has `SPL` 0.329622 and loses to detector-confidence / confidence-only baselines at `SPL` 0.701267. It improves over path-only `SPL` 0.286028 and path-cost ascending `SPL` 0.092750, so the evidence supports a trajectory-cost diagnosis, not a positive navigation-improvement claim. M136 rejects current repair scale-up and selects E008-M137 confidence-preserving trajectory repair: trajectory cost should be a guarded/tie-break signal, not a replacement for confidence ordering. E006-M08 separately rejects human intent as a main contribution under current utility evidence. Final real navigation `SR` / `SPL`, deployable search policy, final RGB-D/open-vocabulary robustness, and human intent as a main contribution remain blocked.
