...
| The system improves real navigation `SR` / `SPL` | simulator/navmesh/trajectory execution and navigation baselines | smoke supported, final H001 claim unsupported | E008-M22 provides Docker-executed detector-policy trajectory smoke over 6 `HM3D ObjectNav` rows. E008-M37 executes 90 dynamic-stale overlay scan-task-policy rows in Docker `Habitat`: detector confidence `SR` 1.0 / `SPL` 0.407894, fixed current top-k `SR` 0.5 / `SPL` 0.373373, H001 task-conditioned memory trust `SR` 0.5 / `SPL` 0.141996, static stale memory `SR` 0.0, task-agnostic memory trust `SR` 0.5 / `SPL` 0.167627. E008-M38 interprets this as repair-before-scale. E008-M39 fixes the budget-matched contract with primary cap 5, source-ready/source-gap separation, and detector all-candidate as upper-bound only. E008-M40 materializes repaired rows. E008-M41 executes them: H001 repaired `SR`/`SPL` 0.5/0.373373, detector/fixed current top-k 0.5/0.373373, task-agnostic 0.5/0.373373, static stale memory 0.0/0.0. E008-M42 confirms scale-up is not justified. E008-M43 fixes source-diverse redesign. E008-M44 materializes 108 execution plans / 468 candidates. E008-M45 fixes Docker-ready contract/preflight and M46 wrapper. This supports only execution readiness, not final navigation improvement |

Latest navigation ledger update: E008-M45 completes source-diverse trajectory execution contract and Docker preflight. Primary budget cap remains 5 stops. The selected method candidate is `h001_task_conditioned_source_diverse_budget5_v1`; the required ablations are `source_diverse_current_observation_budget5_v1` and `task_agnostic_source_diverse_budget5_v1`. M45 selects E008-M46 source-diverse trajectory execution smoke before any broader navigation rerun or positive `SR`/`SPL` claim.
