...
| Boundary | What the paper cannot claim yet | real navigation `SR` / `SPL`, final real RGB-D/open-vocabulary robustness, human intent as a main claim before E006 transfer gates pass in execution |
...
| Structured human intent improves semantic memory decisions | E006 same-evidence paired task-context benchmark, strong context-agnostic baselines, utility/regret metrics, heldout scan/label/task transfer | schema/policy rows ready only | E006-M01 fixes the paired-context/utility contract, E006-M02 fixes strong context-agnostic baselines, required ablations, row schema, and leakage audit, E006-M03 fixes scan/label/task/source/external-route transfer gates, E006-M04 freezes `ContextUtility`, `IntentRegret`, `ContextSpecificGain`, `search_cost_contract_v0`, task profiles, row-generation order, and `implementation_manifest.json` fields, E006-M05 materializes 520 paired-context rows plus 2,600 transfer manifest rows with blocked output term hits 0, and E006-M06 materializes 10,400 frozen baseline policy rows with leakage fail rows 0. Current evidence remains too narrow because utility metrics, strongest context-agnostic comparison, and heldout transfer are not computed yet |
...
- `GOAT-Bench` should wait until E006 fixes the human-intent task suite, because otherwise it will test task/instruction generality before the intended main claim is specified.
