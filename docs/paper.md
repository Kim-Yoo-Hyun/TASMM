...
Latest method-pivot update: E008-M166-M204 are complete. E008-M166-M176 freeze the local-rerank failure boundary and define source coverage as a semantic-map trigger for candidate-source expansion / re-observation before detector-confidence ranking. E008-M177-M184 validate that fixed-budget source-pool acquisition can surface recoverable candidates and execute bounded Docker trajectories on a small branch. E008-M185-M190 show that ranking must remain protected by detector confidence because path/transition-cost orders do not improve `SPL`. E008-M191-M204 scale the source-pool acquisition route, reject immediate trajectory promotion because source-pool replacement lowers full-denominator proxy `SR`, materialize additive union rows without baseline candidate loss, show a leakage-safe proxy improvement over the protected baseline, and materialize the Docker trajectory contract. M204 keeps the method component as additive candidate union with source-gap guard, fixed denominator, fixed primary metric, protected detector-confidence ordering, baseline-prefix audit, dedup audit, leakage audit, and final-claim blocking until executed trajectory results exist.
